...
const DEFAULTS = () => ({
  ...
  A: {
    ...
    focus: 0.2,
    // V9 · ПУЛЬС ▸ ЧАСТОТА — heartbeat RATE multiplier. The whole cloth beats like a
    // chest: faster/deeper in hot spells (high |M| + live threat), near-flatline in
    // dead time. pulseRate scales beats-per-second. 0 = frozen, 1 = tuned default.
    pulseRate: 1.0,
    // V9 · ПУЛЬС ▸ ГЛУБИНА — heartbeat DEPTH multiplier (systolic swell amplitude).
    // Scales how far the surface rises on each beat. The goal punches one huge beat.
    pulseDepth: 1.0,
...
async function init() {
  ...
  buildPulseCurves();        // V9 heartbeat: momentum M(t) + live threat(t)
  ...
  el('title2').textContent =
    `V9 · HEARTBEAT · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function computeField(t, dt) {
  ...
  // ---- V9 HEARTBEAT: advance the pulse for THIS frame ----------------------
  // M(t) drives rate+depth; live threat adds the pre-shot quickening; the goal
  // flood punches one big beat. beatOffset is a UNIFORM low-freq curl term added to
  // every vertex's height below (so it displaces geometry only → no colour band).
  const M_now = pulseMAt(t);
  const absM = Math.abs(M_now);
  const threatNow = pulseThreatAt(t);
  const floodAmtBeat = flood2 && flood2.amt > 0 ? flood2.amt : 0;
  // A_BREATHE = the systolic swell world-Y scale. Larger than the tiny drape wobble
  // so the beat is the HERO channel (felt, not restated). Scales with A.height.
  const A_BREATHE = 0.42 * amp;
  beatOffset = A_BREATHE * updateBeat(t, dt, absM, threatNow, floodAmtBeat);
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      ...
      if (aOn) {
        ...
        // V9 BREATHING — the whole cloth SWELLS with the heartbeat. A single global
        // beat amplitude (beatOffset) shaped by a VERY low-frequency standing curl so
        // the surface reads as one coherent chest rising/falling, not a flat piston.
        // Added UNIFORMLY to both sheets' geometry (no colour term) → the pulse is felt
        // through relief, one meaning per channel. The curl is calmed near the seam so
        // the breath doesn't poke the lap through.
        const breatheShape = 0.72 + 0.28 * Math.sin(u * 2.1 + 0.6) * Math.cos(v * 1.7 - 0.4);
        const breathe = beatOffset * breatheShape;
        ...
        hH = A_BASE + A_WOBBLE * wob + reliefH + breathe;
        hA = A_BASE + A_WOBBLE * wob + reliefA + breathe;
...
// ============================================================================
// V9 · BREATHING FIELD B(t) — the STAR. The whole cloth pulses like a chest. We
// integrate a heartbeat PHASE per real-wall-frame whose RATE rises with |M(t)| and
// live threat (faster, deeper breaths in hot spells; near-flatline in dead time),
// then B(t)=beat(phase) is added UNIFORMLY (a single low-freq curl term) to every
// vertex's height — so it does NOT recolour, one meaning per channel.
//
// The beat waveform is SPRING-like, not a sine drone: a quick systolic SWELL that
// overshoots then settles (organic overshoot-settle), so each beat reads as a
// heartbeat, never a hum. Amplitude tracks depth = pulseDepth·(0.5+1.5·|M|).
//
// A GOAL punches ONE huge held beat synced to the flood sweep, then the post-goal
// lull flatlines the pulse (calm before play resumes) — the visceral flinch.
// ============================================================================
let beatPhase = 0;        // integrated heartbeat phase (radians), wraps each 2π
let beatEnv = 0;          // smoothed breathing amplitude envelope (spring target)
let beatEnvV = 0;         // its velocity (spring, for organic overshoot-settle)
let beatReset = true;     // snap phase/env on scrub/loop
// spring one-beat waveform: rises fast, overshoots, settles — parametrised by the
// wrapped phase p∈[0,2π). A damped-spring-like impulse, NOT a pure sine.
function beatWave(p) {
  // systole: a sharp asymmetric swell early in the cycle; diastole: slow settle.
  const x = p / (Math.PI * 2);          // 0..1 through the cycle
  // fast rise (systole) then damped overshoot-settle (diastole)
  const rise = Math.exp(-x * 5.0);      // energy front-loaded at the start of the beat
  const spring = Math.sin(p * 1.0) + 0.35 * Math.sin(p * 2.0 - 0.6);
  return spring * rise * 2.2;           // ~[-1,1]-ish, front-loaded punch
}
// advance the heartbeat one wall frame; returns the current breathing offset (world-Y
// scale, later × A_WOBBLE·amp). dt = real wall seconds (Infinity on snap → freeze).
function updateBeat(t, dt, absM, threat, floodAmt) {
  const D = Number.isFinite(dt) ? Math.max(0, dt) : 0;   // no time advance on a snap render
  // RATE (beats per wall-second) — near-flatline in dead time, quickening with
  // momentum magnitude AND live threat (the pre-shot quickening). pulseRate scales it.
  const rateK = Number.isFinite(cfg.A.pulseRate) ? clamp(cfg.A.pulseRate, 0, 3) : 1;
  const baseHz = 0.45;                                   // near-flatline resting rate
  const hz = rateK * (baseHz + 1.9 * absM + 1.3 * threat);
  if (D > 0) {
    beatPhase += hz * D * Math.PI * 2;
    if (beatPhase > Math.PI * 2) beatPhase -= Math.PI * 2 * Math.floor(beatPhase / (Math.PI * 2));
  }
  // DEPTH target — deeper breaths when the match is hot; goal punch pushes it to max.
  const depthK = Number.isFinite(cfg.A.pulseDepth) ? clamp(cfg.A.pulseDepth, 0, 3) : 1;
  let target = depthK * (0.5 + 1.5 * absM);
  // GOAL — a huge single systolic PUNCH synced to the flood sweep, then FLATLINE
  // through the lull. floodAmt rises 0→1 in the sweep then eases back; we make the
  // punch peak at the sweep crest and the lull suppress the resting breath.
  const goalPunch = floodAmt * floodAmt;                 // sharpen so it bites at full flood
  target = target * (1 - 0.85 * floodAmt) + goalPunch * (depthK * 3.4);
  if (beatReset || D === 0) { beatEnv = target; beatEnvV = 0; if (beatReset) beatPhase = 0; beatReset = false; }
  else {
    // critically-damped-ish spring toward target so the envelope has organic
    // overshoot-settle rather than a linear lerp.
    const k = 60.0, c = 11.0;
    const a = k * (target - beatEnv) - c * beatEnvV;
    beatEnvV += a * D; beatEnv += beatEnvV * D;
    if (beatEnv < 0) { beatEnv = 0; if (beatEnvV < 0) beatEnvV = 0; }
  }
  return beatWave(beatPhase) * beatEnv;
}
// the breathing offset for the CURRENT frame, published by updateBeat and read in
// the vertex loop (added uniformly to every vertex height). 0 = flat.
let beatOffset = 0;
...
function snapASmoothing() { A_smoothReset = true; focusReset = true; A_frontReset = true; A_frontDispReset = true; locusReset = true; seamTopReset = true; beatReset = true; }
...
// ============================================================================
// V9 · HEARTBEAT PULSE — the TRANSPARENT computed match pulse that drives the
// breathing surface. Two per-bin real-data signals, prebaked so sampling at the
// clock t is deterministic (scrub-safe):
//   pulseM[b]    — own xThreat MOMENTUM M(t) ∈ [-1,1] (+home). Each on-ball
//                  event deposits a threat-weighted impulse (xg + progression +
//                  zone proximity to the attacked goal), signed by team, and the
//                  running home−away difference DECAYS with a half-life. This is
//                  the honest "who is pressing" wave — no valueNorm needed.
//   pulseThr[b]  — live THREAT ∈ [0,1] = max(|home threat|, |away threat|): how
//                  close the ball action is to A goal right now (the pre-shot
//                  quickening). Adds RATE on top of |M|.
// |M| sets the depth+rate of the breathing; threat adds the fast pre-shot
// quickening; a goal punches one huge systolic beat (handled at the flood frame).
// Only real events feed these — no procedural decoration.
// ============================================================================
let pulseN = 0, pulseM = null, pulseThr = null;
const PULSE_DT = 0.1;              // match-minutes per pulse bin
const PULSE_HALFLIFE = 2.4;       // momentum decay half-life (match-minutes)
// how threatening a location is for the attacking team: 0 at own half, →1 at the
// opponent goal mouth. u already mirrored into the shared pitch frame (home attacks
// u→1, away attacks u→0), so we score in each team's own attacking direction below.
function buildPulseCurves() {
  const T = teamMeta.duration || 100;
  const N = Math.max(8, Math.ceil(T / PULSE_DT));
  pulseN = N;
  const rawH = new Float32Array(N);   // per-bin home threat-weighted impulse
  const rawA = new Float32Array(N);   // per-bin away
  const binOf = (t) => clamp(Math.floor(t / PULSE_DT), 0, N - 1);
  for (const e of timeline) {
    // attacking-progress in the team's OWN attacking frame (0 own half → 1 opp goal)
    const atk = e.team === 'home' ? e.u : (1 - e.u);
    // final-third / box proximity weight (smooth): quiet in own half, hot near goal.
    const zone = clamp((atk - 0.5) / 0.5, 0, 1);   // 0 at halfway, 1 at opp goal line
    let w = 0;
    if (e.kind === 'shot') {
      const xg = Number.isFinite(e.xg) ? e.xg : 0;
      w = 0.9 + 6.0 * xg + (e.isGoal ? 4.0 : 0) + 2.2 * zone;
    } else if (e.type === 'Pass') {
      // forward progression into dangerous zones adds momentum
      const prog = Number.isFinite(e.eu) ? clamp((e.team === 'home' ? e.eu - e.u : e.u - e.eu) * 2, 0, 1) : 0;
      w = 0.12 + 0.9 * zone * zone + 0.5 * prog * zone;
    } else if (e.isTouch || ONBALL_TYPES.has(e.type)) {
      w = 0.06 + 0.6 * zone * zone;
    }
    if (w <= 0) continue;
    const b = binOf(e.t);
    if (e.team === 'home') rawH[b] += w; else rawA[b] += w;
  }
  // DECAYING running signals (exp half-life) — threat that fades between events.
  const decay = Math.pow(0.5, PULSE_DT / PULSE_HALFLIFE);
  const runH = new Float32Array(N), runA = new Float32Array(N);
  let ah = 0, aa = 0;
  for (let b = 0; b < N; b++) { ah = ah * decay + rawH[b]; aa = aa * decay + rawA[b]; runH[b] = ah; runA[b] = aa; }
  // normalise each side against a robust high percentile so M lands in ~[-1,1].
  const sorted = Array.from(runH).concat(Array.from(runA)).sort((x, y) => x - y);
  const hi = Math.max(1e-3, sorted[Math.floor(sorted.length * 0.92)] || 1);
  pulseM = new Float32Array(N);
  pulseThr = new Float32Array(N);
  for (let b = 0; b < N; b++) {
    const nh = clamp(runH[b] / hi, 0, 1.4);
    const na = clamp(runA[b] / hi, 0, 1.4);
    pulseM[b] = clamp(nh - na, -1, 1);        // + = home pressing
    pulseThr[b] = clamp(Math.max(nh, na), 0, 1);  // live danger magnitude (either side)
  }
}
// sample M(t) ∈ [-1,1] (linear between bins) — deterministic from the clock.
function pulseMAt(t) {
  if (!pulseM || !pulseN) return 0;
  const f = clamp(t / PULSE_DT, 0, pulseN - 1);
  const i0 = Math.floor(f), i1 = Math.min(i0 + 1, pulseN - 1);
  return lerp(pulseM[i0], pulseM[i1], f - i0);
}
function pulseThreatAt(t) {
  if (!pulseThr || !pulseN) return 0;
  const f = clamp(t / PULSE_DT, 0, pulseN - 1);
  const i0 = Math.floor(f), i1 = Math.min(i0 + 1, pulseN - 1);
  return lerp(pulseThr[i0], pulseThr[i1], f - i0);
}
...
const LAYER_DEFS = [
  { key: 'A', name: 'A · активность', controls: [
    ...
    { id: 'pulseRate', label: 'пульс ▸ частота', min: 0, max: 3, step: 0.05, fmt: (v) => v.toFixed(2) },
    { id: 'pulseDepth', label: 'пульс ▸ глубина', min: 0, max: 3, step: 0.05, fmt: (v) => v.toFixed(2) },
