...
// "EMBER THREAT" — scratch colours reused each frame for the WHITE-HOT goal flashbulb
// (avoids per-frame allocation). See the flood colour path in computeField.
const _fcTeam = new THREE.Color(), _fcClay = new THREE.Color(), _fcScorer = new THREE.Color();
const _fcHot = new THREE.Color(), _fcFlood = new THREE.Color();
...
const DEFAULTS = () => ({
  ...
  A: {
    ...
    // ЖАР ▸ усиление (EMBER GAIN) — "Ember Threat" variant knob. Multiplies the
    // live-threat scalar that feeds uIntensity (the ember magnitude on the mounds
    // near the attacked goal + the xG spire crests). Higher = the forge glows hotter
    // for the same threat. The threat itself is REAL (ball advancement toward the
    // attacked goal + short-τ decaying recent shot xG), so the ember only ignites
    // under live pressure — no decorative heat. 1.0 = as computed; up to ~3.
    emberGain: 1.5,
...
async function init() {
  ...
  el('title2').textContent =
    `EMBER THREAT · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function makeGradientTexture() {
  ...
  // "EMBER THREAT" — very dark, near-monochrome warm-black background (a forge at rest).
  grad.addColorStop(0.0, '#0a0806'); grad.addColorStop(0.55, '#060504'); grad.addColorStop(1.0, '#020101');
...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // "EMBER THREAT" — territories are two BARELY-DISTINGUISHED DARK CLAYS. The clay
    // base is darkened (near-monochrome dark-clay match) and both saturation + tint are
    // dropped so team hue is only WHISPERED. Colour is scarce and reserved for danger:
    // the ONLY loud channel is the ember (uGlowCol × live threat uIntensity).
    uClay: { value: new THREE.Color('#3a352f') },   // dark clay (near-monochrome base)
    uSat: { value: 0.35 },                          // desaturated — team hue barely present
    uTint: { value: 0.5 },                          // low tint — clays are barely distinguished
    uTex: { value: 0.86 },                          // clay micro-texture amount
    uGlowCol: { value: new THREE.Color('#ff7a1e') }, // EMBER — forge-orange heat colour
    uEmber: { value: 1.6 },                          // ember crest strength raised (1.0→1.6)
    uIntensity: { value: 0 },                        // LIVE THREAT at the locus → ember magnitude (the loud channel)
    uPulse: { value: 0 },                            // momentum/pulse magnitude → couples the ember FLICKER (forge breathes hotter when momentum is high)
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = `
      ...
      uniform vec3 uGlowCol; uniform float uEmber; uniform float uIntensity; uniform float uPulse;
      ...
      `#include <emissivemap_fragment>
       {
         ...
         // "EMBER THREAT" — colour is SCARCE. The flat team-colour floor is DIMMED to a
         // faint whisper (0.34→0.12) so territory reads as near-colourless dark clay; the
         // team hue is only barely present. The loud channel is the EMBER, not the team.
         vec3 glowTeam = mix(uTeam, uFloodTeam, clamp(uFlood, 0.0, 1.0));
         vec3 emit = glowTeam * (0.12 * uGlow) * litMul;
         // LIVE-THREAT EMBER — the ONLY vivid thing is heat. Flicker AMPLITUDE is coupled
         // to the pulse/momentum magnitude (uPulse) so the forge "breathes" hotter when
         // momentum is high; base flicker keeps a live shimmer even when calm.
         vec3 Nw = normalize(vNormal);
         float steep = 1.0 - clamp(Nw.y, 0.0, 1.0);
         float flickAmt = 0.16 + 0.34 * clamp(uPulse, 0.0, 1.0);
         float flick = (1.0 - flickAmt) + flickAmt * vn_s10(vUvN * 40.0 + uTime * 0.9);
         float thr = clamp(uIntensity, 0.0, 1.0);
         // (a) MOUND EMBER — the RELIEF drives the ember: near the attacked goal, where
         //     live threat is high, the gentle mounds themselves GLOW orange. Gated by a
         //     low height floor so quiet flat clay stays dark, but any raised mound under
         //     live pressure ignites. Magnitude = live threat (thr) × emberGain (uEmber).
         float moundHot = smoothstep(0.10, 1.1, vHd) * thr;
         // (b) SPIRE BLAZE — xG spires still blaze on their steep faces (existing hot =
         //     smoothstep(1.2,3.0,vHd)), independent of the threat floor so a shot always
         //     ignites a spire.
         float spireHot = smoothstep(1.2, 3.0, vHd) * smoothstep(0.14, 0.6, steep);
         vec3 hi = uGlowCol * (1.0 + smoothstep(2.0, 3.6, vHd) * 0.6);
         float ember = uEmber;
         emit += hi * ember * flick * litMul * (moundHot * 0.9 + spireHot * 1.0);
...
function goalFloodAt(t) {
  ...
  if (elapsed < sweep) {
    ...
    hot = 1;                                                            // WHITE-HOT ignition
  } else if (elapsed < sweep + hold) {
    amt = 1;                                                            // hold full (100%)
    hot = 1;                                                            // still white-hot
  } else {
    ...
    // "EMBER THREAT" FLASHBULB — the goal is a WHITE-HOT full-field ignition that COOLS
    // back to the scorer's dark clay over the RELAX: hot=1 during sweep+hold, eases to 0
    // across the relax so the near-white flash settles into the scorer clay, not a slab.
    hot = 1 - e;
  }
  return { team: g.team, amt: clamp(amt, 0, 1), hot: clamp(hot, 0, 1) };
}
...
// ============================================================================
// "EMBER THREAT" — LIVE THREAT at the locus (the ember MAGNITUDE). Real-data only:
// threat rises only under LIVE PRESSURE. Two honest ingredients, both keyed to the
// ATTACKED goal (home attacks toward u=1, away toward u=0 in the shared home frame):
//   (1) BALL LOCUS ADVANCEMENT — how deep the live ball sits in the attacking third,
//       toward the attacked goal + centred laterally (a chance brews near the goal).
//   (2) RECENT SHOT xG — Σ over recent shots of xg · exp(−Δ/τ_s) with a SHORT τ_s so
//       the heat spikes right at a real shot and cools quickly (a live threat pulse).
// Returns ~0..1 (soft-capped). Deterministic from the clock (event scan) → scrub-safe.
const THREAT_TAU_S = 0.9;   // short decay (match-minutes) → heat only under live pressure
function threatAt(t, ball) {
  // (1) advancement of the live ball toward its attacking goal.
  const uu = clamp(ball.u, 0, 1);
  const adv = ball.team === 'away' ? (1 - uu) : uu;         // 0 = own half, 1 = at attacked goal
  // reward the final third + central positions (where real chances live).
  const depth = smoothstep(0.55, 0.98, adv);
  const central = 1 - smoothstep(0.16, 0.5, Math.abs(clamp(ball.v, 0, 1) - 0.5));
  const posThreat = depth * (0.45 + 0.55 * central);        // ~0..1 from ball locus
  // (2) short-τ decaying sum of recent shot xG (a real shot ignites the forge).
  let shotHeat = 0;
  if (timeline) {
    const lo = t - THREAT_TAU_S * 5;                        // scan a few τ back
    for (const it of timeline) {
      if (it.t > t) break;
      if (it.t < lo) continue;
      if (it.kind !== 'shot') continue;
      const xg = Number.isFinite(it.xg) ? it.xg : 0.05;
      shotHeat += Math.max(xg, 0.04) * Math.exp(-(t - it.t) / THREAT_TAU_S);
    }
  }
  // soft-combine: position sets a live floor, a fresh shot spikes hot on top.
  const raw = 0.7 * posThreat + 1.4 * shotHeat;
...
function computeField(t, dt) {
  ...
  // "EMBER THREAT" FLASHBULB — the goal flood is a WHITE-HOT full-field ignition that
  // COOLS to the scorer's DARK CLAY. flood2.hot=1 during sweep+hold (near-white flash),
  // easing to 0 across the relax. uFloodTeam = mix(scorerDarkClay, near-white, hot), so
  // the field flashes white then settles into the scorer's clay — a flashbulb, not a
  // flat colour slab. scorerDarkClay approximates the sheet's rendered clay (uClay tinted
  // by team at uTint 0.5, desaturated at uSat 0.35) so the cool-down lands on the field.
  const hot = flood2 ? (Number.isFinite(flood2.hot) ? flood2.hot : 0) : 0;
  const scorerTeam = flood2 ? (flood2.team === 'home' ? COL_HOME : COL_AWAY) : COL_HOME;
  _fcTeam.copy(scorerTeam);
  const flum = 0.299 * _fcTeam.r + 0.587 * _fcTeam.g + 0.114 * _fcTeam.b;
  _fcTeam.setRGB(lerp(flum, _fcTeam.r, 0.35), lerp(flum, _fcTeam.g, 0.35), lerp(flum, _fcTeam.b, 0.35));
  _fcClay.set('#3a352f');
  _fcScorer.setRGB(lerp(_fcClay.r, _fcTeam.r, 0.5), lerp(_fcClay.g, _fcTeam.g, 0.5), lerp(_fcClay.b, _fcTeam.b, 0.5));
  _fcHot.set('#fff3e2');   // near-white forge flash (warm, not pure white → bloom without clip)
  _fcFlood.setRGB(lerp(_fcScorer.r, _fcHot.r, hot), lerp(_fcScorer.g, _fcHot.g, hot), lerp(_fcScorer.b, _fcHot.b, hot));
  bH.u.uFlood.value = floodAmt; bA.u.uFlood.value = floodAmt;
  bH.u.uFloodTeam.value.copy(_fcFlood); bA.u.uFloodTeam.value.copy(_fcFlood);
  ...
  // "EMBER THREAT" — LIVE THREAT at the locus feeds the ember MAGNITUDE (uIntensity),
  // scaled by the ЖАР ▸ усиление (emberGain) slider so the forge glows hotter for the
  // same threat. Real-data only: threat = ball advancement toward the attacked goal +
  // short-τ recent shot xG (see threatAt). The ember only ignites under live pressure.
  const emberGain = Number.isFinite(cfg.A.emberGain) ? clamp(cfg.A.emberGain, 0, 3) : 1.5;
  const threat = clamp(threatAt(t, ball) * emberGain, 0, 1);
  bH.u.uIntensity.value = threat; bA.u.uIntensity.value = threat;
  // uEmber = base crest strength × emberGain trim (so the slider drives brightness too).
  const emberBase = Number.isFinite(cfg.A.emberBase) ? cfg.A.emberBase : 1.6;
  bH.u.uEmber.value = emberBase * (0.6 + 0.4 * emberGain);
  bA.u.uEmber.value = emberBase * (0.6 + 0.4 * emberGain);
  // PULSE MAGNITUDE → couples the ember FLICKER (forge breathes hotter when momentum is
  // high). Own xThreat-style pulse magnitude from local event density in a short window;
  // scrub-safe. Kept as a MAGNITUDE (0..1), not signed — the flicker amplitude only.
  const pulseWin = eventsInWindow(t, 0.5);
  const pulseMag = clamp(pulseWin.length / 16, 0, 1);
  bH.u.uPulse.value = pulseMag; bA.u.uPulse.value = pulseMag;
...
function setupComposer() {
  ...
  // "EMBER THREAT" — bloom threshold lowered ONLY enough that the ember crests + the
  // white-hot goal flash bloom (0.52→0.42), strength lifted a touch (0.12→0.18) so the
  // heat glows. ACES roll-off (renderer toneMapping) keeps the ember from clipping.
  bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.18, 0.3, 0.42);
  composer.addPass(bloomPass);
  gradePass = new ShaderPass(GradeShader);
  // "EMBER THREAT" — stronger VIGNETTE + a touch lower exposure so the dark clay stays
  // dark and the ember is the one hot focal point (gallery-grade restraint).
  gradePass.uniforms.uVig.value = 1.45; gradePass.uniforms.uExpo.value = 1.6;
  gradePass.uniforms.uContr.value = 1.14; gradePass.uniforms.uGsat.value = 1.2;
...
const LAYER_DEFS = [
  { key: 'A', name: 'A · активность', controls: [
    ...
    { id: 'emberGain', label: 'жар ▸ усиление (ember)', min: 0, max: 3, step: 0.05, fmt: (v) => v.toFixed(2) },
