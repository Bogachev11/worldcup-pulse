...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    uSat: { value: 0.72 },                          // TECTONIC v1: desaturated clay tint (0.86→0.72) — colour is territory only, chroma reserved for the goal flood
...
function computeField(t, dt) {
  ...
  // TECTONIC v1 — a SECOND, much slower phase drives a global low-frequency wander so
  // the whole landmass rises and settles as ONE breathing organism (on top of the
  // faster fabric ripple). ~0.06× of ph → a several-second breath cycle.
  const phSlow = ph * 0.34;
  ...
  // TECTONIC v1 — MOTION = calm breath tied to the match PULSE. The tiny cloth
  // wobble amplitude is scaled by the momentum magnitude |M(t)| so the WHOLE surface
  // breathes as one organism: quiet passages ≈0.6× (barely moving clay), intense
  // passages ≈1.0× (the landmass swells and settles). Sign of M is unused here.
  const momMag = Math.abs(momAt(t));
  const breath = 0.6 + 0.4 * clamp(momMag, 0, 1);
  const A_WOBBLE = 0.028 * amp * breath;   // tiny cloth wobble, gated by pulse magnitude
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      const u = i / (VX - 1);
      // Fast fabric ripple (drape) + a SLOW, broad curl-like swell shared across the
      // whole sheet (low spatial frequency, slow phase) so the landmass breathes as one
      // organism. The slow term dominates the "one living clay" read; the amplitude of
      // the whole thing is gated by the pulse magnitude via A_WOBBLE.
      const wob = Math.sin(u * 6.1 + ph) * Math.cos(v * 5.3 - ph * 0.8)
                + 0.5 * Math.sin((u + v) * 9.7 - ph * 1.3)
                + 1.2 * Math.sin(u * 1.7 + phSlow) * Math.cos(v * 1.3 - phSlow * 0.7);
...
// TECTONIC v1 — MOMENTUM M(t): a transparent own-xThreat PULSE. Leaky integrator
// over signed, threat-weighted impulses M(t)=Σ signed·exp(−Δ/τ), tanh-squashed to
// [−1,1]. Sign = home(+)/away(−) attacking momentum (kept for other variants,
// UNUSED here). MAGNITUDE |M(t)| drives the global "one organism" breath amplitude
// (see computeField: A_WOBBLE × (0.6+0.4·|M|)). Sampled at DRAMA_DT bins, same as
// the importance curve, and read with momAt(t).
let momM = null;                // Float32Array[dramaN] signed momentum in [-1,1]
const MOM_TAU = 4.0;            // leaky-integrator half-life-ish decay (match-minutes)
...
// TECTONIC v1 — build the sampled momentum curve M(t). A leaky integrator over
// SIGNED, threat-weighted event impulses: each real event deposits an impulse whose
// SIZE is its threat (shots ∝ xg, goals biggest, on-ball/box work smaller) and whose
// SIGN is which team is attacking (home +, away −). Integrating with exp decay gives
// a continuous swing of who is on top; a final tanh squash keeps it in [−1,1]. Only
// the MAGNITUDE is used downstream (the whole surface breathes harder when the match
// is intense), but the sign is computed honestly so sibling variants can reuse it.
function buildMomentum() {
  const N = dramaN || Math.max(8, Math.ceil((teamMeta.duration || 100) / DRAMA_DT));
  const raw = new Float32Array(N);      // signed impulse deposited per bin
  const binOf = (t) => clamp(Math.floor(t / DRAMA_DT), 0, N - 1);
  const dep = (t, w) => { raw[binOf(t)] += w; };
  for (let i = 0; i < timeline.length; i++) {
    const e = timeline[i];
    const sgn = e.team === 'away' ? -1 : 1;   // home attacking = +, away = −
    let w = 0;
    if (e.kind === 'shot') {
      const xg = Number.isFinite(e.xg) ? e.xg : 0;
      w = e.isGoal ? (1.6 + 5.0 * xg) : (0.35 + 3.0 * xg);
      // forward field position (u toward the attacked goal) amplifies threat a touch.
      w *= 1.0 + 0.5 * clamp(e.u, 0, 1);
    } else if (POSSESSION_TYPES.has(e.type) || e.isTouch) {
      // routine on-ball control: a small threat impulse, weighted by how advanced the
      // touch is (deep in the attacking third counts more) so sustained territorial
      // pressure builds momentum, idle midfield barely moves it.
      w = 0.05 * (0.3 + 0.7 * clamp(e.u, 0, 1));
    } else if (DUEL_A_TYPES.has(e.type)) {
      w = 0.03;
    }
    if (w > 0) dep(e.t, sgn * w);
  }
  // leaky integrator M(t)=Σ signed·exp(−Δ/τ): forward pass with per-bin decay.
  const decay = Math.exp(-DRAMA_DT / MOM_TAU);
  const acc = new Float32Array(N);
  let m = 0;
  for (let b = 0; b < N; b++) { m = m * decay + raw[b]; acc[b] = m; }
  // normalise the pre-squash scale so a typical busy passage lands mid-range, then
  // tanh-squash to [−1,1]. Use a robust scale from the 90th-percentile magnitude.
  const mags = [];
  for (let b = 0; b < N; b++) mags.push(Math.abs(acc[b]));
  mags.sort((a, b) => a - b);
  const p90 = mags[Math.min(N - 1, Math.floor(N * 0.9))] || 1e-3;
  const scale = 1.4 / Math.max(p90, 1e-3);
  momM = new Float32Array(N);
  for (let b = 0; b < N; b++) momM[b] = Math.tanh(acc[b] * scale);
}
// Sample the signed momentum at match-minute t (linear interp between bins).
function momAt(t) {
  if (!momM || !momM.length) return 0;
  const N = momM.length;
  const x = clamp(t / DRAMA_DT, 0, N - 1);
  const b = Math.floor(x), f = x - b;
  if (b >= N - 1) return momM[N - 1];
  return momM[b] * (1 - f) + momM[b + 1] * f;
}
...
function buildDramaticClock() {
  ...
  buildMomentum();               // TECTONIC v1: sampled M(t) for the breath amplitude
  const N = dramaN;
