// v5.js — VARIANT #5 "PRESSURE FRONT (cinematic sweep)". CLONED from stage10.js and
// DIVERGED: a wide, low, near-horizon cinematic take where the whole piece is about
// ONE thing — the attacking team's colour TONGUE thrusting toward the far goal on fast
// breaks, hero-lit like a landscape at dusk. Changes vs stage10: LOW hero camera raking
// toward the attacked goal; the thrust finger amplified (cfg.A.thrust ↑) with a bright
// white-hot LEADING EDGE (diffuse + emissive, bloom-picked) that decays on the finger's
// fast τ; a slight LEADING SWELL crest at the tongue's tip (with an anticipatory quiver);
// TIGHTER focus σ so mounds concentrate at the attacked goal; the goal-flood EXHALE
// recedes DIRECTIONALLY as a wave from the scorer's attacked goal back toward their own
// half; stronger vignette + tight high-threshold bloom on the hot edge only. Both matches
// supported via ?id= (1953888 default, 1990967). ONLY real data — no mock.
//
...
// V5 "PRESSURE FRONT" — LOW HERO CAMERA raking toward the ATTACKED (far) goal.
// The dominant home team attacks u→1 (world +X, worldX(1)=+8), so the away goal
// lives at +X. We frame low and near the horizon FROM BEHIND the home half
// (camera at −X, low Y) looking down the pitch toward +X: the colour tongue that
// thrusts toward that far goal reads as a lunge into the distance, hero-lit like a
// landscape at dusk. Y kept low (~6.5) and pushed back/out so the pitch recedes to
// a near-horizon vanishing line rather than a top-down map. Target sits past mid,
// toward the far goal, low, so the horizon rides high in frame.
const DEFAULT_CAM = { pos: [-13.6, 5.4, 5.0], target: [3.2, 0.9, -0.1] };
...
const DEFAULTS = () => ({
  ...
  A: {
    ...
    // V5 PRESSURE FRONT: TIGHTER focus (0.2→0.13) so the mounds concentrate hard at
    // the attacked goal and the surge reads as one crested swell, not a broad field.
    focus: 0.13,
    ...
    // V5 PRESSURE FRONT: thrust is the HERO element — amplified (1.0→1.9) so a fast
    // forward pass stabs a long, dramatic colour tongue toward the far goal.
    thrust: 1.9,
...
async function init() {
  ...
  el('title2').textContent =
    `V5 · PRESSURE FRONT · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // V5 PRESSURE FRONT — bright LEADING EDGE on the thrusting tongue. uThrustGlow
    // 0..1 = how strongly THIS team is currently lunging (derived per-frame from the
    // live thrust-finger weights, so it rises the instant a fast forward pass fires
    // and decays with the finger's own fast τ). uThrustDepth = how far the tongue has
    // stabbed (0..1, front advance toward the attacked goal), used to fade the edge
    // brightness so a bigger lunge glows harder. The brightening is localised to the
    // fabric RIGHT AT this team's leading front (own side of the seam), so it reads as
    // a hot crest on the tip of the colour tongue — the hero element — and nothing
    // else on the sheet changes (restraint elsewhere so the lunge pops).
    uThrustGlow: { value: 0.0 }, uThrustDepth: { value: 0.0 },
    ...
    // V5 PRESSURE FRONT — DIRECTIONAL EXHALE. The goal flood punches in uniformly, but
    // its RELAX recedes as a WAVE from the scorer's attacked goal back toward their own
    // half (echoing the tongue's path). uFloodFront = the receding wavefront position in
    // u (0..1); uFloodDir = +1 when the scorer attacks the high-u goal (home) so the
    // colour clears from u=1 downward, −1 for away (clears from u=0 upward). uFloodSoft =
    // wavefront softness. During sweep/hold the front is parked past both ends so the fill
    // is a full uniform punch; only during relax does it sweep across, so the exhale reads
    // as the flood draining back the way the attack came.
    uFloodFront: { value: 2.0 }, uFloodDir: { value: 1.0 }, uFloodSoft: { value: 0.22 },
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = `
      ...
      uniform float uThrustGlow; uniform float uThrustDepth;
      uniform float uFlood; uniform vec3 uFloodTeam;
      uniform float uFloodFront; uniform float uFloodDir; uniform float uFloodSoft;
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <color_fragment>',
      `#include <color_fragment>
      {
        ...
        // V5 PRESSURE FRONT — BRIGHT LEADING EDGE on the thrusting tongue. When THIS
        // team is lunging (uThrustGlow>0), brighten a thin band of its colour on the
        // OWN side right at the leading front (the tip of the tongue that just stabbed
        // toward the far goal). vDu = u − front(v): this sheet's own side is the side
        // it covers up to the seam. For home (uAway=0) own side is vDu<0; for away
        // (uAway=1) own side is vDu>0. s = signed distance INTO own territory from
        // the front (>=0 on own side). A hot crest sits in the first slice of own
        // territory behind the front and falls off, so only the tongue's leading edge
        // glows — the body of the territory stays the calm two-clay look.
        {
          float s = mix(-vDu, vDu, uAway);                 // + = own side, distance behind front
          // thin bright band hugging the front: full right at s=0, gone by ~edgeW.
          float edgeW = 0.06 + 0.05 * uThrustDepth;         // a longer lunge = a slightly longer crest
          float edge = 1.0 - smoothstep(0.0, edgeW, s);
          edge *= step(0.0, s);                             // only on own side, not lapped past the seam
          float g = clamp(uThrustGlow, 0.0, 1.0) * edge;
          // brighten toward a hot tint of the team colour (leaning white-hot) so the
          // tip reads as a bright leading edge, not just a lighter clay.
          vec3 hotTip = mix(uTeam, vec3(1.0), 0.35) * 1.9;
          col = mix(col, hotTip, clamp(g * 0.85, 0.0, 1.0));
        }
        // GOAL FLOOD — full-field colour override with a V5 DIRECTIONAL EXHALE. The
        // punch fills uniformly (uFloodFront parked past both ends → floodMask=1 for all
        // u), but during the relax the wavefront sweeps across so the colour DRAINS from
        // the scorer's attacked goal back toward their own half, echoing the tongue's
        // path. uFloodDir=+1 → the high-u (home's attacked) end clears first; −1 → the
        // low-u (away's attacked) end clears first. floodMask is 1 behind the front
        // (still flooded) and 0 ahead of it (already receded).
        float floodMask = clamp(smoothstep(uFloodFront - uFloodSoft, uFloodFront + uFloodSoft,
                                           mix(vUvN.x, 1.0 - vUvN.x, step(0.0, uFloodDir))), 0.0, 1.0);
        float floodEff = clamp(uFlood, 0.0, 1.0) * floodMask;
        col = mix(col, uFloodTeam, floodEff);
        ...
        // is lifted as floodEff rises, so both sheets paint fully. As the exhale wave
        // passes a cell (floodEff→0 there) coverage falls back to the honest two-team
        // seam, so the two colours re-emerge from behind the receding flood.
        covEff = max(covEff, floodEff);
      ...
      `#include <emissivemap_fragment>
       {
         ...
         // V5 directional exhale (same wavefront as the diffuse flood) so the emissive
         // floor follows the scorer colour only where the flood still covers.
         float floodMaskE = clamp(smoothstep(uFloodFront - uFloodSoft, uFloodFront + uFloodSoft,
                                             mix(vUvN.x, 1.0 - vUvN.x, step(0.0, uFloodDir))), 0.0, 1.0);
         float floodEffE = clamp(uFlood, 0.0, 1.0) * floodMaskE;
         vec3 glowTeam = mix(uTeam, uFloodTeam, floodEffE);
         vec3 emit = glowTeam * (0.34 * uGlow) * litMul;
         // V5 PRESSURE FRONT — EMISSIVE on the thrusting tongue's leading edge, so the
         // bright tip survives lit shading AND blooms (tight bloom on the tongue only).
         // Same thin own-side band at the front as the diffuse hot-tip above; keyed to
         // uThrustGlow so it rises with the lunge and decays with the finger's fast τ.
         // Suppressed under the flood so a goal reads as one clean full-field colour.
         {
           float s2 = mix(-vDu, vDu, uAway);
           float edgeW2 = 0.06 + 0.05 * uThrustDepth;
           float edge2 = (1.0 - smoothstep(0.0, edgeW2, s2)) * step(0.0, s2);
           float g2 = clamp(uThrustGlow, 0.0, 1.0) * edge2 * (1.0 - clamp(uFlood, 0.0, 1.0));
           vec3 hotTip2 = mix(uTeam, vec3(1.0), 0.4);
           emit += hotTip2 * g2 * 1.15 * litMul;
         }
...
function ensureA(gx, gy) {
  ...
  A_tongueAdv = new Float32Array(gy);   // V5: per-channel signed tongue advance (front − base)
...
// V5 PRESSURE FRONT — per-channel signed tongue advance (front − slow base). + = home
// pushed toward u=1, − = away pushed toward u=0. Drives the leading-swell crest.
let A_tongueAdv = null;
...
function computeA(t, dt) {
  ...
  for (let j = 0; j < gy; j++) {
    ...
    // V5 PRESSURE FRONT — per-channel TONGUE ADVANCE = how far the displayed front has
    // stabbed BEYOND the slow territorial base toward the attacked goal (signed: + =
    // home pushed toward u=1, − = away pushed toward u=0). Feeds the leading-swell crest
    // at the tongue's tip in the vertex loop. Zero when the front sits on the base.
    A_tongueAdv[j] = fr - A_front[j];
    const row = j * gx;
...
function goalFloodAt(t) {
  ...
  if (elapsed < sweep) {
    ...
    relaxF = clamp(f, 0, 1);   // V5: 0..1 across the relax → drives the directional exhale wavefront
  }
  // V5 PRESSURE FRONT — the flood DRAINS from the scorer's attacked goal back toward
  // their own half. home attacks the high-u goal (dir +1 → clear from u=1 down); away
  // attacks the low-u goal (dir −1 → clear from u=0 up). Deterministic from the clock.
  return { team: g.team, amt: clamp(amt, 0, 1), relaxF, dir: g.team === 'home' ? 1 : -1 };
}
...
// V5: linear sample a per-channel array (length gy) at normalized v (same v→row
// convention as sampleGrid: row = (1−v)·(gy−1)).
function sampleChannel(arr, gy, v) {
  const fy = clamp(1 - v, 0, 1) * (gy - 1);
  const j0 = Math.floor(fy), j1 = Math.min(j0 + 1, gy - 1);
  const ty = fy - j0;
  return lerp(arr[j0], arr[j1], ty);
}
...
function computeField(t, dt) {
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      ...
      if (aOn) {
        ...
        // V5 PRESSURE FRONT — LEADING SWELL at the tongue's tip. Where the front has
        // stabbed beyond the slow base (A_tongueAdv[channel] ≠ 0), raise a slight crest
        // in the thrusting team's cloth just BEHIND its front (own side of the seam), so
        // the break reads as a surge with a crest riding forward. Falls off away from the
        // front and scales with how far the tongue advanced. Home advance is +, away −.
        const advCh = sampleChannel(A_tongueAdv, A_gy, v);
        // V5 anticipatory QUIVER — the tip crest trembles slightly as the surge builds,
        // a fast low-amplitude flutter riding on the swell (pulse-like), so a break has
        // a felt anticipation before it consolidates. Fast phase from the wall clock ×
        // channel so adjacent channels shimmer out of step. Tiny (±6%).
        const quiver = 1 + 0.06 * Math.sin(ph * 34.0 + v * 40.0);
        // proximity to the front on the OWN side, ~one lap-width band behind the tip.
        const swellW = Math.max(lap * 3.0, 0.11);
        let swellH = 0, swellA = 0;
        if (advCh > 0.004) {            // home tongue (own side = du<0, just behind front)
          const sH = -du;              // + on home's own side
          if (sH >= 0) {
            const p = 1 - smoothstep(0, swellW, sH);
            swellH = clamp(advCh * 6.0, 0, 1) * p * (0.9 * amp) * reliefMul * quiver;
          }
        } else if (advCh < -0.004) {   // away tongue (own side = du>0)
          const sA = du;
          if (sA >= 0) {
            const p = 1 - smoothstep(0, swellW, sA);
            swellA = clamp(-advCh * 6.0, 0, 1) * p * (0.9 * amp) * reliefMul * quiver;
          }
        }
        // swell rides behind the front; apply a MILD notch (sqrt) so the crest survives
        // near the tip instead of being fully flattened by the seam-crossing notch, while
        // still easing down into the seam so it never pokes through the lip.
        const swellNotch = Math.sqrt(notch);
        const reliefH = (rH * 0.5 * amp * moundMask + xH * crestK * fmCrest) * notch * reliefMul + swellH * swellNotch;
        const reliefA = (rA * 0.5 * amp * moundMask + xA * crestK * fmCrest) * notch * reliefMul + swellA * swellNotch;
  ...
  // V5 PRESSURE FRONT — per-team THRUST-GLOW drive for the bright leading edge. Read
  // the live thrust-finger weights (A_thrustWH/A_thrustWA, built this frame with their
  // OWN fast attack/decay) and the front ADVANCE beyond the slow base (A_frontDisp vs
  // A_front) to measure how hard each team is lunging RIGHT NOW. Max across channels so
  // the strongest active tongue sets the glow; normalise into 0..1. Because the source
  // fingers rise the instant a fast forward pass fires and decay on the finger's fast
  // τ, this glow naturally quivers up and fades with the lunge — no extra state.
  {
    let gH = 0, gA = 0, dH = 0, dA = 0;
    if (aOn && A_thrustWH) {
      for (let j = 0; j < A_gy; j++) {
        // home stabs toward u=1: advance = frontDisp − base (positive when tongue is ahead)
        if (A_thrustWH[j] > 1e-4) {
          const adv = clamp((A_frontDisp[j] - A_front[j]) * 3.0, 0, 1);
          const w = clamp(A_thrustWH[j], 0, 1) * adv;
          if (w > gH) gH = w;
          if (adv > dH) dH = adv;
        }
        // away stabs toward u=0: advance = base − frontDisp
        if (A_thrustWA[j] > 1e-4) {
          const adv = clamp((A_front[j] - A_frontDisp[j]) * 3.0, 0, 1);
          const w = clamp(A_thrustWA[j], 0, 1) * adv;
          if (w > gA) gA = w;
          if (adv > dA) dA = adv;
        }
      }
    }
    bH.u.uThrustGlow.value = gH; bH.u.uThrustDepth.value = dH;
    bA.u.uThrustGlow.value = gA; bA.u.uThrustDepth.value = dA;
  }
  ...
  // V5 PRESSURE FRONT — DIRECTIONAL EXHALE plumbing. During sweep/hold the wavefront is
  // parked below all q (uFloodFront ≪ 0) so the fill is a full uniform punch; during the
  // relax it sweeps 0→1 so the colour drains from the scorer's attacked goal (q=0) back
  // toward their own half (q=1), echoing the tongue's path. q = distance-from-attacked-
  // goal along u (see shader). dir carries which goal is the far one (+1 home, −1 away).
  const floodSoft = 0.2;
  const relaxF = flood2 && Number.isFinite(flood2.relaxF) ? flood2.relaxF : 0;
  const floodDir = flood2 ? (flood2.dir || 1) : 1;
  const relaxE = relaxF <= 0 ? 0 : relaxF * relaxF * (3 - 2 * relaxF);   // ease the sweep
  const floodFront = relaxF > 0 ? lerp(-floodSoft, 1 + floodSoft, relaxE) : -2.0;
  bH.u.uFloodFront.value = floodFront; bA.u.uFloodFront.value = floodFront;
  bH.u.uFloodDir.value = floodDir; bA.u.uFloodDir.value = floodDir;
  bH.u.uFloodSoft.value = floodSoft; bA.u.uFloodSoft.value = floodSoft;
...
function setupComposer() {
  ...
  // V5 PRESSURE FRONT — TIGHT bloom on the tongue's bright edge ONLY: HIGHER threshold
  // (0.52→0.80) so only the white-hot leading edge / xG crest bloom (the calm two-clay
  // field sits below threshold and stays crisp), a TIGHTER radius, modest strength — a
  // hero glow on the lunge, restraint everywhere else.
  bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.16, 0.22, 0.80);
  composer.addPass(bloomPass);
  gradePass = new ShaderPass(GradeShader);
  // V5 — STRONGER vignette (1.28→1.5) to hero-frame the pitch like a dusk landscape;
  // exposure/contrast/sat kept near stage7 so the clay stays matte, not washed.
  gradePass.uniforms.uVig.value = 1.5; gradePass.uniforms.uExpo.value = 1.66;
  gradePass.uniforms.uContr.value = 1.14; gradePass.uniforms.uGsat.value = 1.28;
