...
// v4 PULSE — HARVESTED FotMob momentum, per-minute valueNorm (-1..1, +home). Built
// in buildMomentumM() from /api/rich/{id}; drives the cloth draft/breath. If the
// harvest is missing we fall back to a computed pulse from event/xG density.
let momentumM = null;                       // Float32Array indexed by integer minute, -1..1
...
const DEFAULTS = () => ({
  ...
  A: {
    on: true, open: false, atk: 0.15, rel: 1.6, grid: 0.45, height: 2.0,
...
async function init() {
  ...
  // v4 PULSE — HARVESTED FotMob momentum. Fetch the rich record's per-minute
  // valueNorm (-1..1, +home) and build a minute-indexed M array. This is the
  // variant's distinct pulse source: the CLOTH DRAFT breathes from the real
  // harvested momentum rather than a computed xThreat-style pulse. Fallback (below)
  // to a computed pulse if the rich record / momentum is absent.
  await buildMomentumM();
  countGoals();
  ...
  el('title2').textContent =
    `PAPER WEAVE · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // PAPER WEAVE (v4): a linen/tapestry base rather than clay/stone. Slightly warmer
    // paper-white clay so the two dyed fabrics read as cloth under gallery light.
    uClay: { value: new THREE.Color('#6f6a63') },  // warm linen base (was neutral stone)
    uSat: { value: 0.80 },                          // dyed-fabric tint — slightly desaturated, low contrast
    ...
    uDetail: { value: 1.15 },                        // WEAVE thread depth (a touch deeper so the linen reads)
    uDetailScale: { value: 4.4 },                    // FINE weave — bumped 2.58→4.4 so warp/weft is tight linen
    uPattern: { value: 1 },                          // 1 = WEAVE (linen warp/weft) instead of HEX
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = `
      ...
      float pat_s10(vec2 p){
        ...
        } else if (uPattern < 1.5) {     // WEAVE (v4 linen — over/under warp+weft threads)
          // Two thread families (vertical warp, horizontal weft) as rounded ridges,
          // interleaved over-under so the crosshatch reads as woven cloth, not a soft
          // checker. Each thread is a raised cosine ridge; at each crossing the family
          // whose phase is "up" laps over the other → a real tactile linen weave.
          float warp = 0.5 + 0.5 * cos(p.x * 6.2831853);   // vertical threads (peaks = thread centre)
          float weft = 0.5 + 0.5 * cos(p.y * 6.2831853);   // horizontal threads
          // over-under selector: alternate which family is on top per cell
          float over = 0.5 + 0.5 * sin(p.x * 6.2831853) * sin(p.y * 6.2831853);
          return clamp(mix(min(warp, weft), max(warp, weft), over), 0.0, 1.0);
      ...
      float covAt(){
        ...
        // v4 FRAY: widen the AA cap a touch (0.01→0.018) and add a tiny high-freq
        // per-position jitter to the edge distance so the two dyed fabrics interleave
        // at the seam like woven/frayed edges instead of a razor-clean cut. The jitter
        // is small (fraction of the AA width) and continuous → soft, never a hard notch.
        float aa = clamp(fwidth(vDu), 1e-4, 0.018);
        float fray = (vn_s10(vUvN * 220.0) - 0.5) * aa * 1.1;
        return clamp(smoothstep(-aa, aa, d + fray), 0.0, 1.0);
      }
    ` + shader.fragmentShader;
...
const PITCH_FRAG = `
  ...
  void main(){ float lines = pitchLines7(vUv) * clamp(uLines,0.0,1.0);
    ...
    vec3 lineCol = vec3(0.96,0.93,0.86);  // v4: warm paper-white thread lines (was cool white)
...
// ---- v4 PULSE — HARVESTED FotMob momentum → per-minute M(t) ------------------
// Fetch the rich record and read its `momentum` array of {minute, valueNorm}
// (valueNorm already normalised to -1..1, +home). We fill a minute-indexed
// Float32Array so momentumAt(minute) is a cheap lookup + linear interp. If the
// harvest is absent or empty we synthesise a fallback pulse from the event stream
// (home−away threat density) so the draft still breathes — TRANSPARENT, real-data
// only, no decorative noise.
async function buildMomentumM() {
  let rich = null;
  try { rich = await fetch('/api/rich/' + ID).then((r) => (r.ok ? r.json() : null)); } catch { rich = null; }
  const mom = rich && Array.isArray(rich.momentum) ? rich.momentum : null;
  const dur = Math.max(1, Math.ceil(teamMeta.duration || 100));
  const M = new Float32Array(dur + 2);
  let haveReal = false;
  if (mom && mom.length) {
    for (const p of mom) {
      const mi = Math.round(Number(p.minute));
      if (!Number.isFinite(mi) || mi < 0 || mi > dur + 1) continue;
      const vn = Number(p.valueNorm);
      if (Number.isFinite(vn)) { M[mi] = clamp(vn, -1, 1); haveReal = true; }
    }
  }
  if (!haveReal) {
    // FALLBACK — computed pulse: per-minute home−away threat, xG-weighted, from the
    // real event stream. Same sign convention (+home). Kept transparent + smooth.
    for (let mi = 0; mi <= dur; mi++) {
      let acc = 0;
      for (const it of timeline) {
        const d = mi - it.minute;
        if (d < 0 || d > 3) continue;          // 3-minute decaying window
        const w = Math.exp(-d / 1.4);
        let s = it.kind === 'shot' ? (0.4 + 6 * (it.xg || 0.02)) : (it.isTouch ? 0.05 : 0.02);
        acc += (it.team === 'home' ? s : -s) * w;
      }
      M[mi] = clamp(acc * 0.5, -1, 1);
    }
  }
  momentumM = M;
}
// Linear-interpolated harvested momentum at match-minute t (-1..1, +home).
function momentumAt(t) {
  const M = momentumM;
  if (!M || !M.length) return 0;
  if (t <= 0) return M[0];
  const last = M.length - 1;
  if (t >= last) return M[last];
  const i = Math.floor(t), f = t - i;
  return lerp(M[i], M[i + 1], f);
}
...
function computeA(t, dt) {
  ...
  // v4 SEAM FRAY: a tiny STATIC per-row wobble on the possession seam so the boundary
  // between the two dyed fabrics is a slightly ragged woven edge, not a mathematically
  // straight line. Deterministic (depends only on the row index) → scrub-safe and does
  // not shimmer; amplitude is a fraction of a channel so the tide reading is unchanged.
  for (let j = 0; j < gy; j++) {
    A_frontDisp[j] += (A_frontEff[j] - A_frontDisp[j]) * kd;
    const frayN = Math.sin(j * 12.9898) * 43758.5453;
    const fray = ((frayN - Math.floor(frayN)) - 0.5) * 0.012;   // ±0.6% of pitch length
    const fr = clamp(A_frontDisp[j] + fray, 0, 1);
    const row = j * gx;
...
function computeField(t, dt) {
  ...
  // v4 DRAFT — the gentle cloth-in-draft curl (wob) is the STAR of the motion, and
  // the HARVESTED momentum PULSE modulates its strength: when the match heats up
  // (|M| large) the hung tapestry stirs a touch more in the draft; in lulls it
  // settles almost still. The base drift is SLOWER than stage10 (× 0.8) so the piece
  // feels like fabric settling under gallery light, not a rippling flag.
  const mNow = momentumAt(t);                          // -1..1, +home (harvested valueNorm)
  const draft = 0.6 + 0.9 * Math.abs(mNow);            // pulse → draft strength (0.6 calm … 1.5 lively)
  const ph = (typeof performance !== 'undefined' ? performance.now() : Date.now()) * 0.000088;
  ...
  const A_WOBBLE = 0.032 * amp * draft;  // v4: cloth draft, strength pulsed by harvested momentum
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      ...
      if (aOn) {
        ...
        const crestK = 1.8 * xgH;   // v4: lower cap → xG reads as a raised embroidered knot, not a tower
...
function setupComposer() {
  ...
  bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.07, 0.3, 0.6);  // v4: minimal bloom — gallery-flat, museum vitrine
...
// ============================================================================
const DRAMA_TOTAL_S = 38.0;    // v4: slower pass (~38s) so the tapestry feels like fabric settling under gallery light
...
// ============================================================================
const STORE_KEY = 'wcp_v4_paperweave_cfg';  // v4: own key so it doesn't inherit stage10's saved cfg
