...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // NIGHT VITRINE — the whole thesis is RESTRAINT. Both territories are nearly the
    // SAME neutral clay: uTint dropped LOW so team hue barely tints the base, and uSat
    // muted so what little colour survives is desaturated. The tide seam becomes a
    // tonal/shadow boundary, not a colour boundary. Full team colour appears ONLY at
    // the goal flood (where uFloodTeam is punched to 100% chroma below).
    uSat: { value: 0.60 },                          // muted base saturation (mono clay)
    uTint: { value: 0.35 },                         // faint team tint (near-neutral)
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <color_fragment>',
      `#include <color_fragment>
      {
        ...
        // GOAL FLOOD — uniform full-field OVERRIDE. In NIGHT VITRINE the field is almost
        // colourless, so the flood is THE one moment colour appears — a gong. Locally
        // boost the scorer colour to FULL chroma (100% saturation, ignoring the muted base
        // uSat) so it BRUISES vividly against the neutral clay. Blend the whole cell toward
        // that punched colour by uFlood (same on both sheets) → at uFlood=1 the ENTIRE pitch
        // is the saturated scorer colour, no wave, no seam move.
        float fl = dot(uFloodTeam, vec3(0.299, 0.587, 0.114));
        vec3 floodPop = clamp(mix(vec3(fl), uFloodTeam, 1.55), 0.0, 1.0);  // over-saturate to 100%+ chroma
        col = mix(col, floodPop, clamp(uFlood, 0.0, 1.0));
      ...
      `#include <emissivemap_fragment>
       {
         ...
         // In NIGHT VITRINE the flood colour is punched to full chroma (matches the
         // fragment floodPop) so the goal flood reads as saturated floodlight, not a
         // faint tint. The base territory glow stays gentle/neutral.
         float glf = dot(uFloodTeam, vec3(0.299, 0.587, 0.114));
         vec3 floodGlow = clamp(mix(vec3(glf), uFloodTeam, 1.55), 0.0, 1.0);
         float fA = clamp(uFlood, 0.0, 1.0);
         vec3 glowTeam = mix(uTeam, floodGlow, fA);
         // ramp the glow strength UP during the flood so the punch lands like a gong.
         vec3 emit = glowTeam * ((0.34 + 0.55 * fA) * uGlow) * litMul;
...
function computeField(t, dt) {
  ...
  // NIGHT VITRINE — slow the meditative breath (0.00011→0.00008): stillness is the
  // aesthetic, the pulse is barely perceptible.
  const ph = (typeof performance !== 'undefined' ? performance.now() : Date.now()) * 0.00008;
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      ...
      if (aOn) {
        ...
        // NIGHT VITRINE — RELIEF carries the story (colour is muted), so the xG spires
        // are the main readable structure: taller, cleaner, one focal point per beat.
        const crestK = 3.1 * xgH;
...
function setupComposer() {
  ...
  // NIGHT VITRINE grade: DEEPER vignette (museum spotlight pool), slightly softer
  // exposure, and a muted global saturation to reinforce the near-monochrome look.
  // Full team colour only ever survives at the goal flood (punched in-shader).
  gradePass.uniforms.uVig.value = 1.5; gradePass.uniforms.uExpo.value = 1.66;
  gradePass.uniforms.uContr.value = 1.14; gradePass.uniforms.uGsat.value = 1.02;
  gradePass.uniforms.uGrain.value = 0.055;
  composer.addPass(gradePass);
  smaaPass = new SMAAPass(1, 1); composer.addPass(smaaPass);
  composer.addPass(new OutputPass());
}
const GradeShader = {
  // NIGHT VITRINE grade: deeper vignette + gentle FILM GRAIN. The grain is CRITICAL
  // here — the scene is near-monochrome soft gradients, which band badly on an 8-bit
  // display; a little animated grain dithers the banding away and adds the gallery
  // photographic feel. uGrain = strength, uTime animates it so it never looks static.
  uniforms: {
    tDiffuse: { value: null }, uVig: { value: 0.5 }, uExpo: { value: 1.0 },
    uContr: { value: 1.06 }, uGsat: { value: 1.04 },
    uGrain: { value: 0.05 }, uTime: { value: 0.0 },
  },
  vertexShader: `varying vec2 vUv; void main(){ vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position,1.0); }`,
  fragmentShader: `
    uniform sampler2D tDiffuse; uniform float uVig; uniform float uExpo; uniform float uContr; uniform float uGsat;
    uniform float uGrain; uniform float uTime; varying vec2 vUv;
    ...
    void main(){ vec3 c = texture2D(tDiffuse, vUv).rgb; c *= uExpo;
      ...
      // FILM GRAIN — signed noise per pixel, scrolled by uTime. Kills banding on the
      // near-monochrome clay gradients and gives the photographic gallery-night look.
      float g = grainHash(vUv * vec2(1920.0, 1080.0) + fract(uTime) * 137.0) - 0.5;
      c += g * uGrain;
...
function loop(now) {
  ...
  controls.update();
  // animate the film grain (NIGHT VITRINE) so it dithers banding without looking static.
