...
const DEFAULTS = () => ({
  ...
  A: {
    ...
    colour: 1.0, blur: 0.75, sharp: 1.0, floor: 0.0, lap: 0.05,
    // V6 "TIDAL BASIN" — the two territories are two coloured LIQUIDS in a basin.
    // КРОМКА ▸ подъём — here the lip is EXAGGERATED into a raised MENISCUS along the
    // whole front (the front where the two liquids meet piles up a lip). Higher
    // default so the seam reads as a real fluid meniscus, not a thin fabric fold.
    lipH: 0.26,
    // SLOSH ▸ наклон — the momentum pulse M(t) tilts the whole basin: the liquid
    // level RISES on the side under pressure and FALLS on the other, so the surface
    // leans with momentum. This is the amplitude of that whole-basin tilt (world-Y
    // added/removed at the goal-lines, ×M). 0 = level basin, ~1 = a clear lean.
    slosh: 1.0,
    // THREAT PILE ▸ к воротам — how strongly the threat relief piles toward the
    // ATTACKED goal-line (the liquid heaps against the wall) rather than sitting at
    // the ball. 0 = relief centred on play (old mounds), 1 = fully piled at the wall.
    pile: 0.8,
...
async function init() {
  ...
  el('title2').textContent =
    `TIDAL BASIN · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function fail(msg) {
  const t = el('title2'); if (t) t.textContent = 'TIDAL BASIN · failed: ' + msg;
...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // V6: higher saturation for LIQUID vividness (but still matte, not glossy).
    uSat: { value: 1.05 },                          // liquid-vivid team colour
    ...
    uAO: { value: 0.58 },                            // V6: deeper cavity/valley AO (meniscus depth)
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.vertexShader = `
      ...
      // V6 MENISCUS — a raised LIP of liquid along the WHOLE front, on BOTH sheets
      // (not just the top lap). A smooth rounded bulge straddling the seam: each
      // liquid surface climbs toward the interface like real surface tension, then
      // falls back to its level. Width is a few × the overlap so it reads as a fluid
      // meniscus, not a thin fabric crease. Symmetric in |du|.
      float MENISCUS(float du){
        float w = max(uLap * 3.2, 0.05);                 // meniscus half-width
        float a = clamp(abs(du) / w, 0.0, 1.0);
        // cos-shaped hump: 1 at the seam, 0 at the width edge, smooth everywhere.
        return 0.5 + 0.5 * cos(a * 3.14159265);
      }
    ` + shader.vertexShader;
    ...
    shader.vertexShader = shader.vertexShader.replace('#include <begin_vertex>',
      `#include <begin_vertex>
        ...
        vHd = hb;
        // The TOP sheet folds UP by uLipH near the seam (× smoothed uTop) for the lap.
        // V6: BOTH sheets ALSO raise a MENISCUS lip along the whole front so the two
        // liquids visibly pile up where they meet. The top sheet gets the full fold
        // lap PLUS the meniscus; the under sheet gets a softer meniscus only (~55%).
        vFold = uTop * FOLD(vDu);
        float men = MENISCUS(vDu);
        float meniscusY = uLipH * men * mix(0.55, 1.0, uTop);
        transformed.y += (hb - uBaseline) + uLipH * vFold + meniscusY;`);
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <color_fragment>',
      `#include <color_fragment>
      {
        ...
        // V6 MENISCUS-VALLEY AO — the base of the raised meniscus lip sits in shadow.
        // A ring of darkening just OUTSIDE the seam crest (where the lip curves back
        // down to the liquid level) gives the front real fluid depth. Peaks at a small
        // offset from the seam, fades to nothing at the meniscus edge. Both sheets.
        float mW = max(uLap * 3.2, 0.05);
        float mr = clamp(dist / mW, 0.0, 1.0);
        float valley = smoothstep(0.15, 0.55, mr) * (1.0 - smoothstep(0.7, 1.0, mr));
        col *= mix(1.0, 1.0 - 0.45 * clamp(uAO, 0.0, 1.0), valley);
...
function computeField(t, dt) {
  ...
  // ---- V6 BASIN SLOSH -------------------------------------------------------
  // M(t) ∈ [-1,1]: + = home pressing (attacks u=1), − = away pressing (attacks
  // u=0). It tilts the whole basin so the PRESSED-AGAINST goal side rises (the
  // liquid heaps against the wall being attacked) and the far side falls — a
  // gentle whole-basin lean that follows momentum. The attacked goal-line is
  // u=1 when home presses (M>0) and u=0 when away presses (M<0).
  const M = momentumAt(t, dt);
  const sloshAmp = Number.isFinite(cfg.A.slosh) ? clamp(cfg.A.slosh, 0, 2) : 1;
  // world-Y raised at the attacked goal-line, removed at the defended one. Kept
  // gentle (a few % of the basin) so it reads as a lean, not a ramp. During the
  // post-goal lull the slosh is damped along with the rest of the relief.
  const SLOSH_MAX = 0.9;               // max world-Y lift at the attacked wall
  // tilt(u): linear ramp across the pitch, zero at centre. sign follows M.
  // when M>0 (home presses, attacks u=1): +lift as u→1, −lift as u→0.
  const tiltAt = (u) => (u - 0.5) * 2.0 * M * SLOSH_MAX * sloshAmp;
  ...
  for (let j = 0; j < VY; j++) {
    ...
    for (let i = 0; i < VX; i++, idx++) {
      ...
      if (aOn) {
        ...
        // V6 THREAT PILES TOWARD THE ATTACKED WALL. Instead of a mound centred on
        // the ball, the pressing team's threat relief HEAPS against the goal it is
        // attacking (the liquid piling up high near a goal). attackU = the attacked
        // goal-line u for the currently pressing team (M>0 home→u=1, M<0 away→u=0);
        // wallW rises smoothly to 1 as u approaches that wall. We blend the classic
        // ball-focus mound with this wall pile by cfg.A.pile, gated by |M| so a level
        // match keeps the neutral focus mound.
        const pile = Number.isFinite(cfg.A.pile) ? clamp(cfg.A.pile, 0, 1) : 0.8;
        const attackU = M >= 0 ? 1.0 : 0.0;
        // distance from the attacked wall in u-units, piling within the final ~40%.
        const dWall = Math.abs(u - attackU);
        const wallW = clamp(1 - dWall / 0.42, 0, 1);
        const wallPile = wallW * wallW * (3 - 2 * wallW);   // smooth heap
        const pileMix = pile * clamp(Math.abs(M), 0, 1);
        const focusMound = 0.55 + 0.45 * fm;
        const moundMask = clamp(lerp(focusMound, 0.35 + 0.85 * wallPile, pileMix), 0, 1);
        ...
        // V6 BASIN TILT — the whole liquid surface leans with momentum: the
        // attacked-wall side rises, the far side falls. Added to BOTH sheets so the
        // entire basin (both liquids) tilts as one body. Damped by the post-goal
        // lull (reliefMul) so the goal beat still flattens cleanly.
        const tilt = tiltAt(u) * reliefMul;
        hH = A_BASE + A_WOBBLE * wob + reliefH + tilt;
        hA = A_BASE + A_WOBBLE * wob + reliefA + tilt;
        ...
        // V6: widen the clamp band to cover the MENISCUS zone (lip is now wide),
        // so the under liquid stays cleanly below the top liquid's raised meniscus.
        const seamW = Math.max(lap * 3.4, 0.12);
  ...
  const lipH = clamp(Number.isFinite(cfg.A.lipH) ? cfg.A.lipH : 0.26, 0, 0.45);
  ...
  if (aOn) {
    ...
    for (let k = 0; k < NV; k++) {
      ...
      // V6: mirror the vertex-shader MENISCUS on the TOP sheet so surfYData tracks it.
      const mw = Math.max(lap * 3.2, 0.05);
      const ma = clamp(Math.abs(surfTopDu[k]) / mw, 0, 1);
      const men = 0.5 + 0.5 * Math.cos(ma * Math.PI);
      surfYData[k] = surfTopH[k] - A_DOWN_BIAS + lipH * topUTop * fold + lipH * men;
    }
  } else {
...
// ============================================================================
// V6 · MOMENTUM PULSE M(t) — a TRANSPARENT own-xThreat pulse driving the basin
// SLOSH. For each recent event we add a threat-weighted, exponentially decaying
// impulse: forward-third possession + shots (× xg, goals biggest) push the pulse
// toward the pressing team. Home pushes POSITIVE (home attacks u=1), away NEGATIVE.
// Result is normalised into ~[-1,1] and lightly low-passed so the tilt eases.
// Scrub-safe: purely a function of t over a fixed look-back window.
// ============================================================================
const MOM_WIN = 6.0;      // look-back window in match-minutes
const MOM_TAU = 2.2;      // decay half-scale (minutes) — recent beats weigh most
let _momPrev = NaN;
function momentumRaw(t) {
  ...
  for (const it of timeline) {
    ...
    if (it.t < lo) continue;
    const dtm = t - it.t;
    const decay = Math.exp(-dtm / MOM_TAU);
    // threat weight: how advanced toward the ATTACKED goal + event kind.
    // in the shared frame each team attacks toward u=1 (mirrored), so u is
    // "how advanced" for the acting team; final-third weighs more.
    const adv = clamp(it.u, 0, 1);
    let w = 0;
    if (it.kind === 'shot') {
      w = 0.5 + 3.5 * clamp(it.xg || 0, 0, 1);   // shots dominate; xg scales
      if (it.isGoal) w += 1.5;                     // a goal is the biggest beat
    } else if (POSSESSION_TYPES.has(it.type) || it.isTouch) {
      w = 0.12 * (0.3 + adv * adv);                // forward possession presses
    } else {
      w = 0.04;
    }
    // sign by which team's momentum this is (home +, away −).
    const sign = it.team === 'away' ? -1 : 1;
    acc += sign * w * decay;
  }
  // squash into ~[-1,1]; the divisor sets the pressure scale.
  return clamp(acc / 3.2, -1, 1);
}
// dt-aware low-passed momentum for a smooth lean (snap on scrub via dt=Inf).
function momentumAt(t, dt) {
  const raw = momentumRaw(t);
  const a = expA(dt, 1.2);
  if (!Number.isFinite(_momPrev) || a >= 1) _momPrev = raw;
  else _momPrev += (raw - _momPrev) * a;
  return _momPrev;
}
...
function snapASmoothing() { A_smoothReset = true; focusReset = true; A_frontReset = true; A_frontDispReset = true; locusReset = true; seamTopReset = true; _momPrev = NaN; }
...
const LAYER_DEFS = [
  { key: 'A', name: 'A · активность', controls: [
    ...
    { id: 'lipH', label: 'мениск ▸ подъём', min: 0, max: 0.45, step: 0.01, fmt: (v) => v.toFixed(2) },
    { id: 'slosh', label: 'наклон ▸ импульс', min: 0, max: 2, step: 0.05, fmt: (v) => v.toFixed(2) },
    { id: 'pile', label: 'угроза ▸ к воротам', min: 0, max: 1, step: 0.05, fmt: (v) => v.toFixed(2) },
...
// ============================================================================
const STORE_KEY = 'wcp_v6_tidalbasin_cfg';
