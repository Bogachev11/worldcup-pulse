...
let keyLight;
// V10 — "SUNRISE ARENA" cinematic epilogue. Extra light handles the sun-arc
// animation touches: a warm fill (dawn→dusk) + a cool sky hemisphere whose colours
// and intensities are driven by match PROGRESS (cool dawn at kickoff → warm amber
// dusk at full time). Captured in setupThree so updateSunArc() can animate them.
let fillLight, hemiLight;
// smoothed goal-bloom amount (the arena "lights up" on a goal, then settles).
let sunBloom = 0;
...
async function init() {
  ...
  el('title2').textContent =
    `V10 · SUNRISE ARENA · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function setupThree() {
  ...
  // V10 KEY LIGHT = the "SUN". Its colour + position (elevation) + intensity are
  // animated by updateSunArc() from cool dawn (kickoff) to warm amber dusk (full
  // time). Start at the dawn pose; the arc overwrites it every frame. Shadow frustum
  // WIDENED (long raking shadows as the sun lowers) and softened for cinematic feel.
  keyLight = new THREE.DirectionalLight(0xbcd0ff, 3.1);
  keyLight.position.set(-9, 15, 7);
  keyLight.castShadow = true;
  keyLight.shadow.mapSize.set(2048, 2048);
  keyLight.shadow.camera.near = 1; keyLight.shadow.camera.far = 90;
  const sc = keyLight.shadow.camera;
  // extra-wide frustum so the LONG low-sun shadows (which can rake far across the
  // floating landmass) still fall inside the shadow map and don't clip.
  sc.left = -26; sc.right = 26; sc.top = 20; sc.bottom = -20; sc.updateProjectionMatrix();
  keyLight.shadow.bias = -0.0008; keyLight.shadow.normalBias = 0.04; keyLight.shadow.radius = 7;
  scene.add(keyLight, keyLight.target);

  // WARM FILL — a low, soft counter-light on the shadow side. Colour eases from a
  // faint cool dawn tint to a warm amber bounce, so the dusk end reads golden even
  // in shadow. Position lowered over the match (bounce from a sinking sun).
  fillLight = new THREE.DirectionalLight(0x9fc0ff, 0.55);
  fillLight.position.set(8, 5, -7);
  scene.add(fillLight);

  // SKY HEMISPHERE — ambient tint of the "day". Sky colour cools→warms and the
  // ground bounce follows, so the whole environment tells time, not just the key.
  hemiLight = new THREE.HemisphereLight(0x8fb0e0, 0x0a0d16, 0.5);
  scene.add(hemiLight);
}

// ============================================================================
// V10 · SUNRISE-ARENA LIGHT ARC — the 20s story arcs from COOL DAWN (kickoff, 0')
// to WARM AMBER DUSK (full time). The match plays as a single day passing over the
// floating landmass. Everything here is a function of match PROGRESS (0..1 via
// progressOfMatchT) plus a brief GOAL BLOOM (the arena "lights up" then settles).
// Team territory hues stay TEAM-TRUE (uTint constant) — only the LIGHT tells time.
// ============================================================================
// palette anchors (linear-ish sRGB hexes fed to THREE.Color).
const SUN_KEY_DAWN = new THREE.Color('#a9c4ff');   // cool blue-white low dawn sun
const SUN_KEY_NOON = new THREE.Color('#fff2dc');   // near-white warm midday
const SUN_KEY_DUSK = new THREE.Color('#ff9a3c');   // deep warm amber sunset
const SUN_FILL_DAWN = new THREE.Color('#7f9ed0');  // cool bounce
const SUN_FILL_DUSK = new THREE.Color('#c9702e');  // warm bounce
const SUN_SKY_DAWN = new THREE.Color('#8fb0e0');   // cool sky
const SUN_SKY_DUSK = new THREE.Color('#ffb367');   // amber sky
const SUN_GRD_DAWN = new THREE.Color('#0a0d16');
const SUN_GRD_DUSK = new THREE.Color('#1a0f08');
const _sunC = new THREE.Color();  // scratch
// scratch background regenerated as the sky warms (cheap 16×256 canvas, tinted).
let _bgCanvas = null, _bgTex = null, _bgLastKey = -1;
function skyGradientTexture(prog, bloom) {
  if (!_bgCanvas) { _bgCanvas = document.createElement('canvas'); _bgCanvas.width = 16; _bgCanvas.height = 256; }
  // quantise the regeneration key so we only rebuild the canvas a few dozen times
  // across the pass (not every frame) — smooth enough, cheap.
  const key = Math.round((prog * 60 + bloom * 8));
  if (key === _bgLastKey && _bgTex) return _bgTex;
  _bgLastKey = key;
  const g = _bgCanvas.getContext('2d');
  // top-of-sky: cool navy at dawn → warm dusky orange-brown at full time.
  const top = _sunC.copy(SUN_SKY_DAWN).lerp(SUN_SKY_DUSK, prog).multiplyScalar(0.34 + bloom * 0.5);
  const topHex = '#' + top.getHexString();
  const midC = _sunC.copy(SUN_GRD_DAWN).lerp(SUN_GRD_DUSK, prog);
  const midHex = '#' + midC.getHexString();
  const grad = g.createLinearGradient(0, 0, 0, 256);
  grad.addColorStop(0.0, topHex);
  grad.addColorStop(0.5, midHex);
  grad.addColorStop(1.0, '#020106');
...
function updateSunArc(t, dt) {
  if (!keyLight) return;
  // p = match progress 0 (kickoff) → 1 (full time), the dramatic clock's own arc.
  const p = clamp(progressOfMatchT(t), 0, 1);
  // GOAL BLOOM — the arena briefly lights up warmer/brighter on a goal, then eases
  // back. Tie the target to the flood amt (the celebration punch); smooth so it
  // eases even while the dramatic clock dwells. Snap on scrub (dt = Infinity).
  const fl = goalFloodAt(t);
  const bloomTarget = fl ? fl.amt : 0;
  const kB = (!Number.isFinite(dt)) ? 1 : expA(dt, 0.5);
  sunBloom += (bloomTarget - sunBloom) * kB;
  const bloom = clamp(sunBloom, 0, 1);

  // KEY COLOUR — two-leg ramp dawn→noon→dusk so midday reads bright/neutral and the
  // ends read cool/warm. Goal bloom nudges the hue a touch warmer (celebration glow).
  const legNoon = smoothstep(0, 0.5, p);
  const legDusk = smoothstep(0.5, 1, p);
  _sunC.copy(SUN_KEY_DAWN).lerp(SUN_KEY_NOON, legNoon).lerp(SUN_KEY_DUSK, legDusk);
  if (bloom > 0) _sunC.lerp(SUN_KEY_DUSK, bloom * 0.35);
  keyLight.color.copy(_sunC);
  // KEY INTENSITY — a gentle midday swell, dipping at the cool dawn and the deep
  // dusk (low sun = less flux) + a goal-bloom spike.
  const daylight = 0.72 + 0.5 * Math.sin(Math.PI * clamp(p, 0, 1));  // dim ends, bright mid
  keyLight.intensity = 3.1 * daylight * (1 + bloom * 0.5);

  // KEY POSITION — the sun LOWERS over the match: elevation drops from a high dawn
  // to a low dusk (long raking shadows) and it swings across the arena (azimuth),
  // so the long shadows sweep subtly as the story progresses.
  const elev = lerp(16.5, 3.4, smoothstep(0.05, 1, p));   // world-Y of the sun
  const az = lerp(-Math.PI * 0.62, Math.PI * 0.30, p);    // swing W→E-ish
  const R = 15.5;
  keyLight.position.set(Math.cos(az) * R, elev, Math.sin(az) * R + 2.0);

  // FILL — cool bounce → warm amber bounce; lower + a hair brighter into dusk.
  if (fillLight) {
    _sunC.copy(SUN_FILL_DAWN).lerp(SUN_FILL_DUSK, smoothstep(0.25, 1, p));
    fillLight.color.copy(_sunC);
    fillLight.intensity = 0.5 + 0.25 * p + bloom * 0.35;
    fillLight.position.set(8, lerp(6, 2.5, p), -7);
  }
  // SKY HEMISPHERE — cool → amber; ground bounce warms too. Brighter at dusk so the
  // scene bathes in golden-hour ambient without going flat.
  if (hemiLight) {
    hemiLight.color.copy(_sunC.copy(SUN_SKY_DAWN).lerp(SUN_SKY_DUSK, p));
    hemiLight.groundColor.copy(_sunC.copy(SUN_GRD_DAWN).lerp(SUN_GRD_DUSK, p));
    hemiLight.intensity = 0.46 + 0.20 * p + bloom * 0.25;
  }
  // ENVIRONMENT (IBL) INTENSITY + ACES ROLL-OFF via exposure — the whole render
  // warms and glows toward dusk; a goal briefly spikes the exposure (sunset flare)
  // then eases back. Kept modest so the clay stays MATTE (no plastic sheen).
  if (renderer) {
    renderer.toneMappingExposure = (0.94 + 0.18 * p) * (1 + bloom * 0.28);
  }
  // IBL intensity via the blanket materials' envMapIntensity (works in r0.160;
  // scene-level environmentIntensity was only added later). The clay picks up a
  // touch more warm-ambient bounce toward dusk + on a goal, staying matte.
  if (blankets) {
    const envI = 1.24 * (0.9 + 0.35 * p + bloom * 0.3);
    if (blankets.home) blankets.home.mesh.material.envMapIntensity = envI;
    if (blankets.away) blankets.away.mesh.material.envMapIntensity = envI;
  }
  if (scene) {
    scene.background = skyGradientTexture(p, bloom);
    // deepen the fog toward a warm dusk haze so distant negative space glows amber.
    if (scene.fog) scene.fog.color.copy(_sunC.copy(SUN_GRD_DAWN).lerp(SUN_GRD_DUSK, p).multiplyScalar(1.0 + bloom * 0.4));
  }
}
...
function renderFrame(t, dt) {
  ...
  computeField(t, D);
  // V10 — advance the sunrise→sunset light arc for this clock (cool dawn → warm
  // dusk), plus the goal bloom. Drives lights + exposure + sky, not the geometry.
  updateSunArc(t, D);
}
