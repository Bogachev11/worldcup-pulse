// v2.js — VARIANT #2 "DUEL TIDE" (cloned from stage10.js).
//
// DIVERGENCE FROM stage10: the territory boundary is no longer driven ONLY by ball
// field-position. On top of the existing ball-tide front (A_front), a TRANSPARENT
// MOMENTUM PULSE M(t) — a self-computed leaky-integrator of home−away threat, tanh
// -squashed, deterministic from the clock — globally SHIFTS the whole seam toward the
// leading team. The colour front swings like a tug-of-war rope: "these attacked all
// game, then those scored" is literally readable in how far the seam was pushed each
// moment. A goal against the run shocks because the seam had been pushed the OTHER way.
//
// The momentum-driven shift rides a SPRING (organic overshoot) so the tide visibly
// advances/recedes past its target and settles — you feel pressure as ground taken,
// NOT a bilateral bar restating the game. On a goal the momentum bias SNAPS hard to
// the scorer for the exhale, so the colour RECEDES from the scorer side (a visible
// "the pressure just paid off" recoil). Relief (threat mounds + xG spires) is UNCHANGED
// from the base so threat keeps its own channel and doesn't fight the momentum story.
// Blend is ~50/50 ball-tide vs momentum so it stays football-true, never fully abstract.
//
...
const DEFAULTS = () => ({
  ...
  A: {
    ...
    lull: 1.2,
    // МОМЕНТУМ ▸ сила — MOMENTUM PULSE gain (v2 "Duel Tide" signature knob). The
    // self-computed transparent pulse M(t) ∈ [−1,1] (home−away threat, leaky
    // integrator, tanh) globally SHIFTS the whole seam toward the leading team by
    // ≈ momGain·M(t)·0.25 of pitch length, blended ~50/50 with the ball-tide front.
    // Positive M pushes the seam into the away half; negative into the home half.
    // The shift rides a spring (organic overshoot). 0 = pure ball-tide (base look);
    // 1 = the intended tug-of-war swing; up to 2 = an exaggerated demo swing.
    momGain: 1.0,
...
async function init() {
  ...
  countGoals();
  buildMomentumPulse(tlDoc); // v2: transparent xThreat pulse M(t) (home−away, leaky, tanh)
  ...
  el('title2').textContent =
    `DUEL TIDE · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
// v2 · MOMENTUM-SEAM SPRING — the momentum offset that globally shifts the whole seam
// does NOT jump to its target M(t); it rides a damped spring so the tide visibly
// ADVANCES/RECEDES with organic OVERSHOOT and settles. momDisp = current spring pos
// (in u-units, applied to every channel's front), momVel = spring velocity. On a
// goal we SNAP the target hard to the scorer for the exhale (see momTargetAt) so the
// colour recoils FROM the scorer side. Reset (snap to target) on scrub/resize.
let momDisp = 0, momVel = 0, momReset = true;
...
function computeA(t, dt) {
  ...
  // ---- v2 · MOMENTUM SEAM SHIFT (the "Duel Tide" signature) -------------------
  // Advance the damped SPRING toward this frame's momentum target (momTargetAt), then
  // add its position as a GLOBAL offset to every channel's front so the WHOLE seam
  // swings toward the leading team — blended ~50/50 with the ball-tide so it stays
  // football-true (never fully abstract). The spring gives organic OVERSHOOT: the tide
  // visibly advances past its mark and settles, like a tug-of-war rope. On scrub/resize
  // we SNAP to the target (no phantom swing on a jump-cut). dt-aware so it's stable at
  // any frame rate; deterministic target → the settled state is reproducible.
  const momTgt = momTargetAt(t);
  if (momReset || !Number.isFinite(momDisp)) {
    momDisp = momTgt; momVel = 0; momReset = false;
  } else {
    const dtc = Math.min(Math.max(dt, 1e-3), 0.05);         // clamp step for stability
    const OMEGA = 9.0;                                       // spring stiffness (rad/s)
    const ZETA = 0.55;                                       // damping (<1 → overshoot)
    const accel = OMEGA * OMEGA * (momTgt - momDisp) - 2 * ZETA * OMEGA * momVel;
    momVel += accel * dtc;
    momDisp += momVel * dtc;
  }
  // BLEND 50/50: only half of the spring offset is applied so the ball-tide keeps its
  // vote — the boundary is a genuine blend of WHERE PLAY IS and WHO HAS MOMENTUM.
  const momOff = momDisp * 0.5;
  const bandLo = clamp(Number.isFinite(cfg.A.ownBand) ? cfg.A.ownBand : 0, 0, 0.45);
  const bandHi = 1 - bandLo;
  for (let j = 0; j < gy; j++) {
    let fr = A_front[j];
    ...
    // GLOBAL momentum offset, then re-clamp so neither own-goal band is ever erased.
    fr = clamp(fr + momOff, bandLo, bandHi);
    A_frontEff[j] = fr;   // COMBINED front this frame (eased base + fingers + momentum).
                          // NO flood wash — the flood is a uniform colour override.
  }
...
function snapASmoothing() { A_smoothReset = true; focusReset = true; A_frontReset = true; A_frontDispReset = true; locusReset = true; seamTopReset = true; momReset = true; }
...
// ============================================================================
// v2 · MOMENTUM PULSE M(t) — the PRIMARY territory driver of "Duel Tide".
// A TRANSPARENT, self-computed xThreat-style pulse: home−away, threat-weighted,
// leaky-integrator (decaying accumulation) squashed with tanh into [−1,1]. Precomputed
// deterministically into a per-bin array so momentumAt(t) is a pure function of the
// clock → scrub-safe and snap-safe (no frame-to-frame state).
//
// M(t) > 0  ⇒ home has the momentum (the pressure) ⇒ seam pushes into the AWAY half.
// M(t) < 0  ⇒ away has the momentum ⇒ seam pushes into the HOME half.
//
// Threat weight per event (signed +home / −away):
//   · shot ∝ (base + xG), goals weightiest, on-target bonus — the clearest threat.
//   · box / final-third entries, dangerous forward passes into the attacking third,
//     and dribbles in the final third — the build-up pressure BETWEEN shots, so the
//     tide swings during a siege even before a shot lands.
// Deposited as impulses, then run through a LEAKY INTEGRATOR (exp decay per bin) both
// FORWARD (so a burst of pressure keeps the tide pushed for a while and then leaks back)
// — this is what makes the boundary "swing like a tug-of-war rope" rather than twitch.
let momBins = null;          // Float32Array of M(t) in [−1,1], one per DRAMA_DT bin
let momN = 0;
function buildMomentumPulse(doc) {
  const T = teamMeta.duration || 100;
  const N = Math.max(8, Math.ceil(T / DRAMA_DT));
  momN = N;
  const raw = new Float32Array(N);   // signed threat impulses per bin (+home / −away)
  const binOf = (t) => clamp(Math.floor(t / DRAMA_DT), 0, N - 1);
  // Use the RAW doc events (they carry x/endX/situation/xg in the attacking frame per
  // team) so we can read forward progress + zone without the mirrored pitch frame.
  const evs = Array.isArray(doc.events) ? doc.events : [];
  for (const e of evs) {
    const team = e.team === 'home' || e.team === 'away' ? e.team : null;
    if (!team) continue;
    const sgn = team === 'home' ? 1 : -1;
    const t = Number(e.t) || 0;
    const xg = Number.isFinite(e.xg) ? e.xg : 0;
    let w = 0;
    const isShot = e.type === 'SavedShot' || e.type === 'MissedShots' || e.type === 'ShotOnPost' || e.type === 'Goal' || e.isGoal;
    if (isShot) {
      // the clearest threat signal. Goals dominate, on-target > off-target.
      const onTarget = (e.type === 'SavedShot' || e.type === 'ShotOnPost' || e.type === 'Goal' || e.isGoal);
      w = 2.2 + 9.0 * xg + (onTarget ? 1.4 : 0) + (e.isGoal ? 6.0 : 0);
    } else if (e.type === 'Pass' || e.type === 'TakeOn' || e.type === 'BallTouch') {
      // build-up PRESSURE between shots: reward events that occur in / advance into the
      // attacking third (x is already in this team's attacking frame, 0..100 → own→opp
      // goal). Forward passes gaining ground near the box are momentum, sideways in own
      // half is not. Keeps the tide swinging during a siege.
      const x = Number.isFinite(e.x) ? e.x : 50;
      const ex = Number.isFinite(e.endX) ? e.endX : x;
      const zone = clamp((Math.max(x, ex) - 55) / 45, 0, 1);   // 0 at halfway, 1 at opp goal
      if (zone > 0) {
        const gain = clamp((ex - x) / 40, -0.5, 1);            // forward progress fraction
        const through = (e.through || e.cross) ? 0.5 : 0;
        w = zone * (0.35 + 0.9 * Math.max(0, gain) + through);
        if (e.type === 'TakeOn') w *= 1.3;                     // a dribble in the final third
      }
    }
    if (w > 0) raw[binOf(t)] += sgn * w;
  }
  // LEAKY INTEGRATOR (forward exponential accumulation): pressure builds and then leaks
  // back over ~HALF match-minutes. This gives the tide its ORGANIC swing/inertia.
  const acc = new Float32Array(N);
  const HALF_MIN = 3.0;                                        // leak half-life (match-min)
  const decay = Math.exp(-DRAMA_DT / (HALF_MIN / Math.LN2));   // per-bin retention
  let s = 0;
  for (let b = 0; b < N; b++) { s = s * decay + raw[b]; acc[b] = s; }
  // NORMALISE the leaky sum to a comfortable scale, then tanh-squash into [−1,1] so a
  // dominant spell saturates near ±1 but never clips hard. Scale by the RMS of |acc|
  // so both a lopsided match (FRA-SEN) and an open one (ICO-NOR) read on the same band.
  let sq = 0, cnt = 0;
  for (let b = 0; b < N; b++) { sq += acc[b] * acc[b]; cnt++; }
  const rms = Math.sqrt(sq / Math.max(1, cnt)) || 1;
  const k = 1 / (rms * 1.35);                                  // tanh input gain
  momBins = new Float32Array(N);
  for (let b = 0; b < N; b++) momBins[b] = Math.tanh(acc[b] * k);
}
// momentumAt(t) — M(t) ∈ [−1,1] at clock match-minute t (linear-interpolated between
// bins). Pure function of t → scrub-safe. +1 = home fully dominant, −1 = away.
function momentumAt(t) {
  if (!momBins || momN <= 0) return 0;
  const bf = clamp(t / DRAMA_DT, 0, momN - 1);
  const b = Math.floor(bf), f = bf - b;
  const b1 = Math.min(b + 1, momN - 1);
  return lerp(momBins[b], momBins[b1], f);
}
// v2 · momTargetAt(t) — the seam-offset TARGET (u-units) the spring chases at clock t.
// Base = momGain · M(t) · MOM_SPAN. On a goal, for the flood exhale window, we OVERRIDE
// the base and SNAP the target hard TOWARD the scorer's own half — i.e. the seam recoils
// FROM the scorer side ("the pressure just paid off"). Sign convention matches the front:
// a POSITIVE offset pushes the seam toward u=1 (into the away half = home pressure).
// So a HOME goal wants the seam to recede toward u=0 → NEGATIVE recoil; an AWAY goal →
// POSITIVE recoil. Deterministic from t (goalFloodAt is pure) → scrub-safe.
const MOM_SPAN = 0.25;              // max fraction of pitch length the pulse shifts the seam
function momTargetAt(t) {
  const momGain = Number.isFinite(cfg.A.momGain) ? clamp(cfg.A.momGain, 0, 2) : 1;
  let target = momGain * momentumAt(t) * MOM_SPAN;
  const fl = goalFloodAt(t);
  if (fl && fl.amt > 0.02) {
    // RECOIL: during the goal flood/exhale, bias the seam toward the SCORER's own goal
    // (away FROM their attacking half). Strength tracks the flood envelope so the recoil
    // punches in with the goal and relaxes with the flood. Home scorer → seam toward u=0
    // (negative); away scorer → seam toward u=1 (positive).
    const recoil = (fl.team === 'home' ? -1 : 1) * MOM_SPAN * 1.25 * fl.amt;
    // blend from the momentum target into the recoil as the flood asserts, so the swing
    // that had been building doesn't just vanish — it's visibly YANKED the other way.
    target = lerp(target, recoil, clamp(fl.amt, 0, 1));
  }
  return target;
}
...
const LAYER_DEFS = [
  { key: 'A', name: 'A · активность', controls: [
    ...
    { id: 'momGain', label: 'моментум ▸ сила прилива', min: 0, max: 2, step: 0.05, fmt: (v) => v.toFixed(2) },
