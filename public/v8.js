// v8.js — VARIANT #8 "RIDGE LINES (relief-primary, topo-map)".
//
// The match as a hand-shaded RELIEF MAP — a cartographer's football match. The
// same working engine as stage10 (two team blankets, possession tide, xG spires,
// dramatic time, goal flood), but the RELIEF is now read like a topographic survey:
//   · HYPSOMETRIC CONTOUR BANDING on the relief — thin darker bands at height
//     intervals (from vHd) so the mounds + xG spires read as surveyed elevation.
//     The taller the spire, the more rings → a shot's danger is COUNTABLE at a glance.
//     Contours ride the RELIEF channel only; they do NOT recolour team territory.
//   · A MUTED, slightly desaturated MAP palette (two region tints), even IBL.
//   · A slow, map-like calm: an own-xThreat-style MOMENTUM pulse M(t) raises/lowers
//     the whole relief "SEA LEVEL", so the contours breathe outward under pressure.
//   · GOAL = a uniform flood punch during which the contour lines WASH OUT, then the
//     map "redraws" — contours return as the relief recovers on the exhale.
//
// Scaffolding cloned from stage10.js. ONLY real data — no mock, no procedural
// decoration. The contour term is HEIGHT-DERIVED ONLY (fract(vHd·N) → thin dark
// band), uniform per height, so it stays on the relief read and never becomes a
// zero→non-zero TEAM colour band.
...
async function init() {
  ...
  el('title2').textContent =
    `V8 · RIDGE LINES · ${teamMeta.home.abbr} ${teamMeta.score.home}–${teamMeta.score.away} ${teamMeta.away.abbr}`;
...
function makeBlanket(teamCol, isAway) {
  ...
  const u = {
    ...
    // MUTED MAP PALETTE (V8) — a paler survey-sheet paper base and a lower tint/sat
    // so the two territories read as soft, slightly desaturated MAP REGIONS (like a
    // relief atlas), not vivid clay. The team hue is still recognisable but calmed.
    uClay: { value: new THREE.Color('#b9b3a6') },  // pale survey-paper base (muted, warm-grey)
    uSat: { value: 0.5 },                           // desaturated map tint (was 0.86)
    uTint: { value: 0.62 },                         // gentler team tint (regions, not neon; was 1.0)
    uTex: { value: 0.7 },                           // clay micro-texture amount (a touch softer)
    uGlowCol: { value: new THREE.Color('#f0d8c1') }, // ember crest colour
    uEmber: { value: 0.5 },                          // ember crest strength — dimmed for the calm map look
    // ---- V8 HYPSOMETRIC CONTOURS ----
    // uContourN = rings per world unit of relief height (a few per unit → a spire
    // gets several countable rings). uSeaLevel = global relief offset from the
    // momentum pulse M(t) (raises/lowers the whole terrain so contours breathe
    // outward under pressure). uContourFade = 1 normally, →0 during the goal flood
    // (contours wash out, then redraw as the relief recovers).
    uContourN: { value: 3.0 },
    uSeaLevel: { value: 0.0 },
    uContourFade: { value: 1.0 },
  ...
  mat.onBeforeCompile = (shader) => {
    ...
    shader.vertexShader = `
      ...
      uniform float uSeaLevel;                 // V8: global relief "sea-level" offset from M(t)
    ...
    shader.vertexShader = shader.vertexShader.replace('#include <begin_vertex>',
      `#include <begin_vertex>
        ...
        // V8 SEA-LEVEL BREATH — the momentum pulse M(t) raises/lowers the whole relief.
        // Gate the lift by where relief actually EXISTS (smoothstep on hb) so flat map
        // territory stays flat (contours only live on the mounds/spires) and the terrain
        // breathes: under pressure the mounds swell and their contour rings push outward.
        float reliefGate = smoothstep(0.02, 0.6, hb);
        float sea = uSeaLevel * reliefGate;
        float hbSea = hb + sea;            // sea-lifted relief → drives BOTH geometry & contours
        vHd = hbSea;                       // fragment contours read the breathing height
        ...
        vFold = uTop * FOLD(vDu);
        transformed.y += (hbSea - uBaseline) + uLipH * vFold;`);
    shader.fragmentShader = `
      ...
      uniform float uTime;
      uniform float uContourN; uniform float uContourFade;   // V8 hypsometric contours
      varying float vHd; varying vec2 vUvN; varying float vDu; varying float vFold;
    ...
    shader.fragmentShader = shader.fragmentShader.replace('#include <color_fragment>',
      `#include <color_fragment>
      {
        ...
        // ---- V8 HYPSOMETRIC CONTOUR BANDING -------------------------------------
        // Read the (sea-lifted) relief height vHd like a topographic survey: a thin
        // darker band at every 1/uContourN of height. fract(vHd·N) is the fractional
        // position within the current height interval; a smoothstep V-notch around the
        // interval edge draws a soft dark BAND (not a hairline — respects the
        // no-tiny-detail gallery rule). The taller the spire, the MORE bands it crosses
        // → a shot's danger is COUNTABLE. This is HEIGHT-DERIVED ONLY and uniform per
        // height, so it never becomes a zero→non-zero TEAM colour band. Contours fade
        // near flat cloth (no rings on the calm map) and wash out during the goal flood
        // (uContourFade → 0), redrawing as the relief recovers on the exhale.
        {
          float cN = max(uContourN, 0.5);
          float phase = fract(vHd * cN);                 // 0..1 within a height interval
          float edge = min(phase, 1.0 - phase);          // 0 at a contour line, →0.5 mid-band
          // BAND (not hairline): dark within ~12% of the interval around each edge,
          // softened so it reads as a printed contour, not aliased 1px lines.
          float lineW = 0.12;
          float contour = 1.0 - smoothstep(0.0, lineW, edge);   // 1 on a line → 0 mid-band
          // only where relief exists (rings live on the mounds/spires, not flat map).
          float reliefMask = smoothstep(0.06, 0.5, vHd);
          // darken the region tint by the contour (max ~38% darker on the line).
          float band2 = contour * reliefMask * uContourFade;
          col *= mix(1.0, 0.62, clamp(band2, 0.0, 1.0));
        }
...
// ============================================================================
// V8 MOMENTUM PULSE M(t) — a TRANSPARENT own-xThreat-style pulse computed from the
// real event stream (no FotMob field needed): each recent threatening event deposits
// a threat weight that DECAYS with age, signed +home / −away. Shots (esp. high xG,
// goals) weigh most; forward/final-third passes add gentle pressure. Summed and
// normalised to ~[-1,1] (+ = home pressing, − = away). This drives the relief
// "SEA-LEVEL" breath: when a team presses, |M| rises, the whole relief swells and the
// contour rings push outward. Deterministic from t (recomputed from the event window
// each frame) → scrub-safe. Only real data.
const MOM_HALF_MIN = 2.2;        // threat half-life (match-minutes) — a slow, map-like breath
function momentumAt(t) {
  if (!timeline) return 0;
  const relMin = MOM_HALF_MIN / Math.LN2;
  const win = eventsInWindow(t, relMin * 4);
  let sum = 0;
  for (const e of win) {
    const age = t - e.t;
    if (age < 0) continue;
    const dec = Math.exp(-age / relMin);
    let w = 0;
    if (e.kind === 'shot') {
      const xg = clamp(e.xg || 0, 0, 1);
      w = (e.isGoal ? 5.0 : 1.5) + 6.0 * xg;      // shots dominate the pulse
    } else if (e.kind === 'pass' && Number.isFinite(e.eu)) {
      const adv = e.eu - e.u;                      // forward gain (attacking frame)
      if (adv > 0.05) w = 0.5 * clamp(adv * 2.0, 0, 1) + (e.eu > 0.66 ? 0.4 : 0);
    } else if (POSSESSION_TYPES.has(e.type) || e.isTouch) {
      w = 0.06;                                    // faint on-ball pressure
    }
    if (w <= 0) continue;
    sum += (e.team === 'home' ? 1 : -1) * w * dec;
  }
  // normalise: divide by a nominal busy threat scale so a strong press → ~±1.
  return clamp(sum / 7.0, -1, 1);
}
let momSmooth = 0, momReset = true;   // eased momentum so the sea-level breath is gentle
...
function computeField(t, dt) {
  ...
  // ---- V8 SEA-LEVEL BREATH + CONTOUR FADE ----------------------------------
  // The momentum pulse M(t) raises/lowers the whole relief "sea level" so the
  // contours breathe outward under pressure. Ease it so the breath is slow/map-like
  // (snap on scrub via momReset). |M| drives the swell (either team pressing lifts
  // the terrain); the pulse is intensity, not a signed tilt, for the sea level.
  const mRaw = momentumAt(t);
  const kMom = momReset ? 1 : expA(dt, 0.6); momReset = false;
  momSmooth += (mRaw - momSmooth) * kMom;
  // sea level: a gentle baseline lift + extra under pressure. Scaled to amp so the
  // rings shift a few intervals, not a violent heave. During the goal lull the relief
  // is flattening anyway, so keep the sea calm then (× reliefMul).
  const seaLevel = (0.12 + 0.5 * Math.abs(momSmooth)) * clamp(amp, 0, 8) * 0.16 * reliefMul;
  bH.u.uSeaLevel.value = seaLevel; bA.u.uSeaLevel.value = seaLevel;
  // CONTOUR FADE — contours WASH OUT during the goal flood (uFlood→1) and REDRAW as
  // the relief recovers on the exhale. (1−floodAmt) drives the wash; the map redraws
  // naturally as floodAmt returns to 0. Kept full otherwise.
  const contourFade = clamp(1 - floodAmt * 1.15, 0, 1);
  bH.u.uContourFade.value = contourFade; bA.u.uContourFade.value = contourFade;
